...
static Asset* current_asset(FlipperReconApp* app) {
...
void flipper_recon_scene_asset_edit_on_enter(void* context) {
    FlipperReconApp* app = context;
    ...
    variable_item_list_set_selected_item(
        list, scene_manager_get_scene_state(app->scene_manager, FlipperReconSceneAssetEdit));

    view_dispatcher_switch_to_view(app->view_dispatcher, ReconViewVarItemList);
}

bool flipper_recon_scene_asset_edit_on_event(void* context, SceneManagerEvent event) {
    FlipperReconApp* app = context;
    bool consumed = false;

    if(event.type == SceneManagerEventTypeCustom) {
        scene_manager_set_scene_state(app->scene_manager, FlipperReconSceneAssetEdit, event.event);
        switch(event.event) {
        case AssetEditName:
        ...
        case AssetEditEvidence:
            scene_manager_next_scene(app->scene_manager, FlipperReconSceneEvidenceList);
            consumed = true;
            break;
...
void flipper_recon_scene_asset_edit_on_exit(void* context) {
    FlipperReconApp* app = context;
