...
static void kyber_series_check_read_callback(LFRFIDWorkerReadResult result, ProtocolId protocol,
                                             void* context) {
    DisneyToolboxApp* app = context;
    if (result == LFRFIDWorkerReadDone) {
        app->protocol_id = protocol;
        view_dispatcher_send_custom_event(app->view_dispatcher, DisneyToolboxEventReadDone);
    }
}

void disney_toolbox_app_scene_kyber_series_check_on_enter(void* context) {
    DisneyToolboxApp* app = context;
    Popup* popup = app->popup;

    popup_set_icon(popup, 5, 2,
                   kyber_read_animation[app->animation_counter / kyber_read_animation_mod]);
    ...
    lfrfid_worker_read_start(app->lfworker, LFRFIDWorkerReadTypeAuto,
                             kyber_series_check_read_callback, app);

    notification_message(app->notifications, &sequence_blink_start_cyan);
}

bool disney_toolbox_app_scene_kyber_series_check_on_event(void* context, SceneManagerEvent event) {
    DisneyToolboxApp* app = context;
    bool consumed = false;

    if (event.type == SceneManagerEventTypeCustom) {
        if (event.event == DisneyToolboxEventReadDone) {
            ...
            for (size_t i = 0; i < CrystalSeries1_MAX; i++) {
                if (CrystalsSeries1[i].em4100[0] == em_hi &&
                    ...
                    if (strcmp(CrystalsSeries1[i].hilt_color, CrystalsSeries1[i].blade_color) ==
                        0) {
                        furi_string_cat_printf(result, "Color: %s\n",
                                               CrystalsSeries1[i].hilt_color);
                    } else {
                        furi_string_cat_printf(result, "Blade: %s   Hilt: %s\n",
            ...
            for (size_t i = 0; i < CrystalSeries2_MAX; i++) {
                if (CrystalsSeries2[i].em4100[0] == em_hi &&
                    ...
                    furi_string_cat_printf(result, "\n- %s (%s)", CrystalsSeries2[i].crystal_color,
...
void disney_toolbox_app_scene_kyber_series_check_on_exit(void* context) {
    DisneyToolboxApp* app = context;
    notification_message(app->notifications, &sequence_blink_stop);
