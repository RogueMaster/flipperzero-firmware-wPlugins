...
// Known badge tapped (via Punch or "Use badge"): the punch is fully automatic -
// it alternates IN / OUT based on the last event (first ever = IN). No manual
// choice: we record immediately and show a short confirmation.
...
void timeclock_scene_badge_action_on_enter(void* context) {
    ...
    // Automatic alternation: OUT if the last punch was IN, otherwise IN.
...
bool timeclock_scene_badge_action_on_event(void* context, SceneManagerEvent event) {
    ...
    if(event.type == SceneManagerEventTypeCustom && event.event == SCENE_EVENT_DONE) {
