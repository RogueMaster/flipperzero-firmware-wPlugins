...
static const Icon* kyber_success_animation[] = {
    &I_lightsaber_frame_0, &I_lightsaber_frame_1, &I_lightsaber_frame_2,
    &I_lightsaber_frame_3, &I_lightsaber_frame_4,
};

void disney_toolbox_app_scene_kyber_write_success_on_enter(void* context) {
    DisneyToolboxApp* app = context;
    Popup* popup = app->popup;

    popup_set_header(popup, "Success!", 75, 10, AlignLeft, AlignTop);
    popup_set_icon(popup, 10, 9,
                   kyber_success_animation[app->animation_counter / kyber_success_animation_mod]);
    popup_set_context(popup, app);
    popup_set_callback(popup, disney_toolbox_app_popup_timeout_callback);
...
bool disney_toolbox_app_scene_kyber_write_success_on_event(void* context, SceneManagerEvent event) {
    DisneyToolboxApp* app = context;
    bool consumed = false;

    if (event.type == SceneManagerEventTypeBack || event.type == SceneManagerEventTypeCustom) {
        scene_manager_search_and_switch_to_another_scene(
            app->scene_manager, app->kyber_series == 2 ? DisneyToolboxAppSceneKyberSelectorS2
                                                       : DisneyToolboxAppSceneKyberSelectorS1);
        consumed = true;
...
void disney_toolbox_app_scene_kyber_write_success_on_exit(void* context) {
    DisneyToolboxApp* app = context;
