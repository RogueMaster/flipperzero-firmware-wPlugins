...
static bool dash_prestart_tick(SigRoamApp* app) {
    ...
    confirmed = sr_dialect_show_info_clear_done(
        app->model.wifi_stop_rev, app->clear_stop_rev);
    timed_out =
        (uint32_t)(furi_get_tick() - app->dash_prestart_tick_ms) >=
...
static void dash_scan_toggle(SigRoamApp* app) {
    ...
    act = sr_scan_ctl_on_ok_upload_gate(
        ...
        sr_scan_ctl_ident_hold(
            ...
            furi_get_tick(),
            (uint32_t)SR_SCAN_CTL_IDENT_MS),
        sr_scan_ctl_uploading(
            app->model.firmware.diag_seen, app->model.firmware.diag_state));
    act = sr_scan_ctl_retry_unconfirmed(
        act,
        st,
        app->model.session == SrSessionRunning,
        app->scan.cmd_is_start);
    if(act == SrScanActSendStart) {
        ...
        if(clear == SrShowInfoClearWait &&
           sr_dialect_show_info_clear_done(
               app->model.wifi_stop_rev, app->clear_stop_rev)) {
            dash_queue_cmd(app, true);
            return;
        }
    ...
    if(event.type == SceneManagerEventTypeTick) {
        ...
        dash_qual_refresh_tick(app);
        {
            ...
            SrAlertKind ak = sr_alert_eval(
                &app->alert, app->model.gps_csv_rev, app->model.gps_csv.fix, now_ms);
            if(ak != SrAlertNone) {
