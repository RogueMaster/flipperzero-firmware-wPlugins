...
static void disney_toolbox_app_scene_kyber_selector_s2_confirm_dialog_callback(
    DialogExResult result, void* context) {
    DisneyToolboxApp* app = context;
    view_dispatcher_send_custom_event(app->view_dispatcher, result);
}

static void kyber_selector_s2_update_status_text(DisneyToolboxApp* app) {
    ...
    dialog_ex_set_left_button_text(dialog_ex, app->selected_s2 > CrystalSeries2_01 ? "Prev" : NULL);
    dialog_ex_set_right_button_text(dialog_ex,
                                    app->selected_s2 < CrystalSeries2_MAX - 1 ? "Next" : NULL);
    dialog_ex_set_center_button_text(dialog_ex, "Write");

    dialog_ex_set_result_callback(
        dialog_ex, disney_toolbox_app_scene_kyber_selector_s2_confirm_dialog_callback);
    dialog_ex_set_context(dialog_ex, app);
}

void disney_toolbox_app_scene_kyber_selector_s2_on_enter(void* context) {
    DisneyToolboxApp* app = context;
    app->kyber_series = 2;
...
bool disney_toolbox_app_scene_kyber_selector_s2_on_event(void* context, SceneManagerEvent event) {
    DisneyToolboxApp* app = context;
...
void disney_toolbox_app_scene_kyber_selector_s2_on_exit(void* context) {
    DisneyToolboxApp* app = context;
