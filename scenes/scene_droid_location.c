...
static void loc_config_enter_callback(void* context, uint32_t index) {
    ...
    app->beacon_data_len = droid_location_beacon_generate(
        app->selected_droid_location, loc_interval_info[app->droid_loc_interval_idx].value,
        loc_rssi_info[app->droid_loc_rssi_idx].value, app->beacon_data);
    scene_manager_set_scene_state(app->scene_manager, DisneyToolboxAppSceneDroidLocation, index);
    scene_manager_next_scene(app->scene_manager, DisneyToolboxAppSceneDroidLocationBroadcast);
}

void disney_toolbox_app_scene_droid_location_on_enter(void* context) {
    DisneyToolboxApp* app = context;
    VariableItemList* list = app->var_item_list;
    VariableItem* item;
    uint32_t item_count = 0;

    item = variable_item_list_add(list, "Location", DroidLocationCount, loc_config_location_changed,
                                  app);
    variable_item_set_current_value_index(item, app->selected_droid_location);
    variable_item_set_current_value_text(item,
                                         droid_location_info[app->selected_droid_location].name);
    item_count++;

    item = variable_item_list_add(list, "Interval", LOC_INTERVAL_COUNT, loc_config_interval_changed,
                                  app);
    variable_item_set_current_value_index(item, app->droid_loc_interval_idx);
    variable_item_set_current_value_text(item, loc_interval_info[app->droid_loc_interval_idx].name);
    item_count++;
    ...
    uint32_t selected =
        scene_manager_get_scene_state(app->scene_manager, DisneyToolboxAppSceneDroidLocation);
...
bool disney_toolbox_app_scene_droid_location_on_event(void* context, SceneManagerEvent event) {
    UNUSED(context);
    UNUSED(event);
    return false;
}

void disney_toolbox_app_scene_droid_location_on_exit(void* context) {
    DisneyToolboxApp* app = context;
