    ...
    if(AP->playing == false){
        ...
        for(int i = 0; i < PROJECTILES; i++){
            if(AP->apples[i]->playing == true){
                canvas_draw_icon(canvas, AP->apples[i]->coordinate.y, AP->apples[i]->coordinate.x, apple);
            }
        }
    }
                ...
                if(event.type == InputTypePress) {
                    ...
                    for(int i = 0; i < PROJECTILES; i++){
                        if (AP->apples[i]->playing){
                            if(AP->apples[i]->coordinate.x > 113 && AP->apples[i]->coordinate.y >= AP->player->y && AP->apples[i]->coordinate.y <= (AP->player->y + 10)){
