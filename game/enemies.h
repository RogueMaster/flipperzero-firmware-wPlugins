...
void enemiesUpdate()
{
    ...
    if (bitRead(spikes[i].characteristics, 2)) // spike active
    {
      ...
      if (!bitRead(spikes[i].characteristics, 0)) {
        for (int l = 8; l < spikes[i].pos.height; l += 8)
          gfx_sprite_overwrite(commonx, commony + l, sprSpikes,  spikes[i].characteristics & 0x03);
      }
      else {
        for (int l = 8; l < spikes[i].pos.width; l += 8)
          gfx_sprite_overwrite(commonx + l, commony, sprSpikes,  spikes[i].characteristics & 0x03);
      }
    }
  }
  ...
  for (uint8_t i = 0; i < MAX_PER_TYPE; ++i)
  {
      ...
      if (everyXFrames(2))
        for (uint8_t a = 0; a < MAX_FAN_PARTICLES; ++a)
        {
          ...
          fans[i].particles[a].y =
            ...
            fans[i].particles[a].y + 6 : randomOf((fans[i].height) >> 2);

          // Draw particles
          switch (fans[i].dir)
          {
            case FAN_UP:
            gfx_sprite_erase(fans[i].pos.x + fans[i].particles[a].x - cam.pos.x, fans[i].pos.y - fans[i].particles[a].y - cam.pos.y, particle , 0);
            break;
            case FAN_RIGHT:
            gfx_sprite_erase(fans[i].pos.x + 16 + fans[i].particles[a].y - cam.pos.x, fans[i].pos.y + 16 - fans[i].particles[a].x - cam.pos.y, particle , 0);
            break;
            default:
            gfx_sprite_erase(fans[i].pos.x - fans[i].particles[a].y - cam.pos.x, fans[i].pos.y + 16 - fans[i].particles[a].x - cam.pos.y, particle , 0);
          }
        }
    ...
    if (walkers[i].active)
    {
      ...
      gfx_sprite_overwrite(walkers[i].pos.x - cam.pos.x, walkers[i].pos.y - cam.pos.y, walkerSprite, walkerFrame + (walkers[i].HP <= 0) * 2);
    }

    // Coins
    if (coins[i].active)
    {
      gfx_sprite_overwrite(coins[i].pos.x - cam.pos.x, coins[i].pos.y - cam.pos.y, elements, coinFrame);
    }
  }
}
