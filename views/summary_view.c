...
static void summary_view_draw(Canvas* canvas, void* model) {
    ...
    canvas_set_font(canvas, FontSecondary);
    snprintf(value, sizeof(value), "%lu %s", (unsigned long)s->baud, hermes_framing_name(s->framing));
    ...
    if(session_stats_silent(s)) {
        ...
        canvas_draw_str_aligned(
            canvas, 64, 44, AlignCenter, AlignTop, "the line stayed quiet");
        return;
    }
