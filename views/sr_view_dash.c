 ...
 */
static int32_t sr_view_dash_put_big(
    Canvas* canvas, uint32_t v, const char* label, int32_t max_baseline) {
    char num[12];
    size_t h;
    uint16_t wnum;
    uint16_t wlab;
...
static void sr_view_dash_draw_dash(Canvas* canvas, const SrDashModel* m) {
    ...
    if(m->wait_stage != (uint8_t)SrWaitStageNone &&
       !sr_scan_ctl_ident_yields_state4(
           ...
           sr_scan_ctl_sd_dead(
               m->qual_rev, m->qual.sd, m->qual_tick_ms, furi_get_tick()),
           m->board_sealing)) {
...
static void sr_view_dash_draw_gps(Canvas* canvas, const SrDashModel* m) {
    ...
    {
        bool qfresh =
            sr_fmt_qual_fresh(m->qual_rev, m->qual_tick_ms, m->sess_ms, furi_get_tick());
