...
static bool disney_toolbox_app_custom_event_callback(void* context, uint32_t event) {
    furi_assert(context);
    DisneyToolboxApp* app = context;
    return scene_manager_handle_custom_event(app->scene_manager, event);
}

static bool disney_toolbox_app_back_event_callback(void* context) {
    furi_assert(context);
    DisneyToolboxApp* app = context;
    return scene_manager_handle_back_event(app->scene_manager);
}

static void disney_toolbox_app_tick_event_callback(void* context) {
    furi_assert(context);
    DisneyToolboxApp* app = context;
    scene_manager_handle_tick_event(app->scene_manager);
}

static void disney_toolbox_app_init_beacon_config(DisneyToolboxApp* app) {
...
static DisneyToolboxApp* disney_toolbox_app_alloc(void) {
    DisneyToolboxApp* app = malloc(sizeof(DisneyToolboxApp));

    app->gui = furi_record_open(RECORD_GUI);
    app->notifications = furi_record_open(RECORD_NOTIFICATION);

    app->scene_manager = scene_manager_alloc(&disney_toolbox_app_scene_handlers, app);
    ...
    view_dispatcher_set_navigation_event_callback(app->view_dispatcher,
                                                  disney_toolbox_app_back_event_callback);
    view_dispatcher_set_tick_event_callback(app->view_dispatcher,
                                            disney_toolbox_app_tick_event_callback, 100);
    view_dispatcher_attach_to_gui(app->view_dispatcher, app->gui, ViewDispatcherTypeFullscreen);

    app->submenu = submenu_alloc();
    view_dispatcher_add_view(app->view_dispatcher, DisneyToolboxAppViewSubmenu,
                             submenu_get_view(app->submenu));

    app->dialog_ex = dialog_ex_alloc();
    view_dispatcher_add_view(app->view_dispatcher, DisneyToolboxAppViewDialog,
                             dialog_ex_get_view(app->dialog_ex));

    app->widget = widget_alloc();
    view_dispatcher_add_view(app->view_dispatcher, DisneyToolboxAppViewWidget,
                             widget_get_view(app->widget));

    app->popup = popup_alloc();
    view_dispatcher_add_view(app->view_dispatcher, DisneyToolboxAppViewPopup,
...
static void disney_toolbox_app_free(DisneyToolboxApp* app) {
    view_dispatcher_remove_view(app->view_dispatcher, DisneyToolboxAppViewSubmenu);
    view_dispatcher_remove_view(app->view_dispatcher, DisneyToolboxAppViewDialog);
...
int32_t disney_toolbox_app(void* args) {
    UNUSED(args);

    DisneyToolboxApp* app = disney_toolbox_app_alloc();

    scene_manager_next_scene(app->scene_manager, DisneyToolboxAppSceneMenu);

    view_dispatcher_run(app->view_dispatcher);

    disney_toolbox_app_free(app);
    return 0;
}

void disney_toolbox_app_popup_timeout_callback(void* context) {
    DisneyToolboxApp* app = context;
    view_dispatcher_send_custom_event(app->view_dispatcher, DisneyToolboxEventPopupClosed);
}
