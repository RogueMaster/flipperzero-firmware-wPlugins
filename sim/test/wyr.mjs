...
let now = 0, out = [];
for (let s = 0; s < 4; s++) { now += 1000; out = e.tick(now); }
assert.equal(lastToWs(out, 1).msg.phase, "vote", "round 1 opens for voting after the countdown");
...
const late = lastToWs(e.join(9, "ZED"), 9).msg;
...
// Play again wipes it.
const again = lastToWs(e.input(1, { t: "again" }), 1).msg;
