...
#define MENU_VISIBLE                                                           \
  5 // 菜单可见行数 (选中项永远在中间第3行, 即 MENU_SEL_IDX=2)
#define MENU_SEL_IDX 2
#define MENU_ROW_H 10
#define MENU_Y_START                                                           \
  19 // 标题+分隔线后开始 (y=17 分隔线, 到 52 底分隔 = 35px 可用)
...
// 根据 msg_id 返回对应中文位图
static void get_msg_bmp(int id, const uint8_t **bits, int *w, int *h,
                        int *bpr) {
  switch (id) {
  case MSG_KEY:
    *bits = msg_key_bits;
...
static void zh_story_line(int sid, int page, int line, const uint8_t **bits,
                          int *w, int *h) {
...
static void draw_story(Canvas *c) {
  ...
  if (g.lang == LANG_ZH) {
    ...
    // 页码 (数字 ASCII)
    char pg[16];
    snprintf(pg, sizeof(pg), "%d/%d", g.story_page + 1,
             story_pages(g.story_id));
    ...
    if (g.story_page < np - 1) {
      canvas_draw_xbm(c, 2, 62 - STORY_HINT_H + 1, STORY_HINT_W, STORY_HINT_H,
                      story_hint_bits);
    } else {
      // 最后一页: 选项 A/B 位图 + 光标
      int selY = (g.story_choice == 0) ? 48 : 58;
      canvas_draw_str(c, 0, selY, ">");
      canvas_draw_xbm(c, 8, 48 - CHOICE_A_H + 1, CHOICE_A_W, CHOICE_A_H,
                      choice_a_bits);
      canvas_draw_xbm(c, 8, 58 - CHOICE_B_H + 1, CHOICE_B_W, CHOICE_B_H,
                      choice_b_bits);
    }
    return;
  }
...
static void draw_inventory(Canvas *c) {
  ...
  if (has_quest && s_inv_page == 1) {
    ...
    for (int i = 0; i < g.quest.sub_count && y < 56; i++) {
      ...
      if (g.lang == LANG_ZH) {
        ...
        if (lb)
          canvas_draw_xbm(c, 14, y, lw, lh, lb);
      } else {
  ...
  // 页码指示 (有任务时显示 1/2)
  if (has_quest)
    canvas_draw_str_aligned(c, 126, 16, AlignRight, AlignTop, "1/2");
  ...
  if (g.lang == LANG_ZH) {
    ...
    for (int i = 0; i < ITEM_COUNT; i++) {
      ...
      // 光标 >
      if (i == g.inv_sel)
        canvas_draw_str(c, 0, y + 8, ">");
      if (nb)
        canvas_draw_xbm(c, 10, y, nw, nh, nb);
    ...
    if (has_quest)
      ...
      canvas_draw_xbm(c, 2, 62 - INV_HINT_H + 1, INV_HINT_W, INV_HINT_H,
                      inv_hint_bits);
    return;
  }
...
static void draw_level_select(Canvas *c) {
  ...
  if (g.lang == LANG_ZH) {
    ...
    for (int lvl = 1; lvl <= g.ls_max; lvl++) {
      ...
      if (y + row_h <= y_top || y >= y_bot)
        continue;
      bool locked =
          g.ls_for_campaign && (lvl > g.campaign_cleared + 1) && !g.dev_mode;
      ...
      if (tag_b)
        canvas_draw_xbm(c, 113, y - 1, tag_w, tag_h, tag_b);
      canvas_set_color(c, ColorBlack);
    }
    // 进度条
    if (g.ls_max > 1) {
    ...
    canvas_draw_xbm(c, 2, 62 - LS_HINT_H + 1, LS_HINT_W, LS_HINT_H,
                    ls_hint_bits);
    return;
  }
  ...
  canvas_set_font(c, FontPrimary);
  canvas_draw_str_aligned(c, 64, 1, AlignCenter, AlignTop,
                          g.ls_for_campaign ? EN_LS_TITLE_STORY
                                            : EN_LS_TITLE_ENDLESS);
  canvas_set_font(c, FontSecondary);
  {
    int y_top = LS_Y_START, y_bot = LS_Y_END;
...
static void draw_opening(Canvas *c) {
  ...
  // ===== Stage 2: 副标题从底部滑入 =====
    ...
    if (g.lang == LANG_ZH)
      ...
      canvas_draw_xbm(c, (128 - OPEN_BY_W) / 2, sub_y, OPEN_BY_W, OPEN_BY_H,
                      open_by_bits);
  }
  // ===== Stage 3: logo + 副标题静止, 等待开场自然结束进入菜单 =====
    ...
    if (g.lang == LANG_ZH)
      ...
      canvas_draw_xbm(c, (128 - OPEN_BY_W) / 2, 50, OPEN_BY_W, OPEN_BY_H,
                      open_by_bits);
  }
}
...
// 档位表 (双语)
static const char *LBL_TURN_SENS[] = {"1.0x",  "1.25x", "1.5x",
                                      "1.75x", "2.0x",  "2.5x"};
static const char *LBL_SHORT_DEG[] = {"5.7d", "8.6d", "11.5d", "14.3d",
                                      "17.2d"};
static const char *LBL_MOVE_SHT[] = {"0.08", "0.12", "0.15", "0.20", "0.26"};
static const char *LBL_MOVE_MAX[] = {"0.024", "0.030", "0.042", "0.055",
                                     "0.072"};
static const char *LBL_TURN_MAX[] = {"0.030", "0.038", "0.050", "0.065",
                                     "0.085"};
...
static const char *LBL_MCBLK_ZH[] = {"砖", "石", "木",   "草",
                                     "土", "沙", "原木", "叶"};
static const char *LBL_MCBLK_EN[] = {"Brk", "Stn", "Wd",  "Grs",
                                     "Drt", "Snd", "Log", "Lef"};
...
static const SetEntry SIMPLE_SETS[] = {
    SET_SIMPLE(SET_BOOL, &g.sfx_enabled, 0, 1, LBL_ONOFF_ZH, LBL_ONOFF_EN, NULL,
               "音效", "Sound FX"),
    SET_SIMPLE(SET_BOOL, &g.opening_enabled, 0, 1, LBL_ONOFF_ZH, LBL_ONOFF_EN,
               NULL, "开场动画", "Opening"),
};
...
static const SetEntry DEV_SETS[] = {
    // 控制类 (7)
    {SET_VAL8, &g.cfg_turn_sens, 0, 5, LBL_TURN_SENS, LBL_TURN_SENS, NULL,
     "转向灵敏度", "Turn Sens"},
    {SET_VAL8, &g.cfg_turn_short, 0, 4, LBL_SHORT_DEG, LBL_SHORT_DEG, NULL,
     "短按转角", "Turn Short"},
    {SET_VAL8, &g.cfg_move_short, 0, 4, LBL_MOVE_SHT, LBL_MOVE_SHT, NULL,
     "短按步幅", "Move Short"},
    {SET_VAL8, &g.cfg_move_max, 0, 4, LBL_MOVE_MAX, LBL_MOVE_MAX, NULL,
     "移动速度", "Move Speed"},
    {SET_VAL8, &g.cfg_turn_max, 0, 4, LBL_TURN_MAX, LBL_TURN_MAX, NULL,
     "转向速度", "Turn Speed"},
    {SET_VAL8, &g.cfg_jump_height, 0, 3, LBL_JUMP_PX_ZH, LBL_JUMP_PX_EN, NULL,
     "跳跃高度", "Jump Hgt"},
    {SET_VAL8, &g.cfg_back_ratio, 0, 3, LBL_BACK_RT, LBL_BACK_RT, NULL,
     "后退速度", "Back Ratio"},
    // 画面类 (5)
    {SET_VAL8, &g.cfg_density, 0, 2, LBL_DENSITY_ZH, LBL_DENSITY_EN, NULL,
     "渲染密度", "Density"},
    {SET_BOOL, &g.cfg_fog, 0, 1, LBL_ONOFF_ZH, LBL_ONOFF_EN, NULL, "距离雾效",
     "Fog"},
    {SET_VAL8, &g.cfg_brightness, 0, 4, LBL_BRIGH, LBL_BRIGH, NULL, "亮度",
     "Brightness"},
    {SET_BOOL, &g.cfg_sky_ceil, 0, 1, LBL_ONOFF_ZH, LBL_ONOFF_EN, NULL,
     "天空天花板", "Sky/Ceiling"},
    {SET_BOOL, &g.cfg_floor_tex, 0, 1, LBL_ONOFF_ZH, LBL_ONOFF_EN, NULL,
     "地板纹理", "Floor Tex"},
    // 音效类 (3)
    {SET_VAL8, &g.cfg_sfx_vol, 0, 2, LBL_VOL_ZH, LBL_VOL_EN, NULL, "音量",
     "Volume"},
    {SET_BOOL, &g.cfg_sfx_menu, 0, 1, LBL_ONOFF_ZH, LBL_ONOFF_EN, NULL,
     "菜单音效", "Menu SFX"},
    {SET_BOOL, &g.cfg_sfx_combat, 0, 1, LBL_ONOFF_ZH, LBL_ONOFF_EN, NULL,
     "战斗音效", "Combat SFX"},
    // 游戏参数 (5)
    {SET_VAL8, &g.cfg_maze_scale, 0, 4, LBL_MAZE_SC, LBL_MAZE_SC, NULL,
     "迷宫缩放", "Maze Scale"},
    {SET_VAL8, &g.cfg_hp_start, 0, 4, LBL_HP, LBL_HP, NULL, "初始血量",
     "Start HP"},
    {SET_VAL8, &g.cfg_regen_rate, 0, 3, LBL_REGEN, LBL_REGEN, NULL, "回血速度",
     "Regen Rate"},
    {SET_VAL8, &g.cfg_ammo_mul, 0, 3, LBL_AMMO, LBL_AMMO, NULL, "弹药倍率",
     "Ammo Mul"},
    {SET_VAL8, &g.cfg_endless_start, 0, 4, LBL_ENDLESS, LBL_ENDLESS, NULL,
     "无尽起始", "Endless Strt"},
    // MC 沙盒 (4)
    {SET_VAL8, &g.cfg_mc_size, 0, 3, LBL_MCSZ, LBL_MCSZ, NULL, "MC地图大小",
     "MC Size"},
    {SET_VAL8, &g.cfg_mc_day_len, 0, 3, LBL_MCDAY, LBL_MCDAY, NULL,
     "MC日夜速度", "MC Day Len"},
    {SET_BOOL, &g.cfg_mc_jump, 0, 1, LBL_ONOFF_ZH, LBL_ONOFF_EN, NULL, "MC跳跃",
     "MC Jump"},
    {SET_VAL8, &g.cfg_mc_start_sel, 0, 7, LBL_MCBLK_ZH, LBL_MCBLK_EN, NULL,
     "MC初始方块", "MC Block"},
    // 调试 (1)
    {SET_BOOL, &g.show_debug, 0, 1, LBL_ONOFF_ZH, LBL_ONOFF_EN, NULL,
     "调试信息", "Debug Info"},
};
...
static const SetEntry *settings_get(int idx) {
  if (idx < (int)SIMPLE_SET_COUNT)
    return &SIMPLE_SETS[idx];
  idx -= (int)SIMPLE_SET_COUNT;
  if (idx < (int)DEV_SET_COUNT)
    return &DEV_SETS[idx];
  return NULL;
}
...
static void settings_val_str(const SetEntry *e, char *buf, int bufsize) {
  ...
  if (e->type == SET_BOOL) {
    snprintf(buf, bufsize, "%s",
             zh ? LBL_ONOFF_ZH[v ? 1 : 0] : LBL_ONOFF_EN[v ? 1 : 0]);
    return;
  }
  // SET_VAL8: 优先用档位标签
...
static void draw_settings(Canvas *c) {
  ...
  if (g.dev_mode) {
    char tb[32];
    if (g.lang == LANG_ZH)
      snprintf(tb, sizeof(tb), "开发者设置 %d/%d", s_set_sel + 1,
               settings_count());
    else
      snprintf(tb, sizeof(tb), "Dev Settings %d/%d", s_set_sel + 1,
               settings_count());
  ...
  if (g.dev_mode) {
    static const struct {
      ...
      const char *en;
        ...
        {0, 1, "基础", "Basic"},   {2, 8, "操控", "Control"},
        {9, 13, "画面", "Video"},  {14, 16, "音效", "Audio"},
        {17, 21, "游戏", "Game"},  {22, 25, "MC", "MC"},
        {26, 26, "调试", "Debug"},
    };
  ...
  if (g.lang == LANG_ZH) {
    canvas_draw_str(c, 2, 62,
                    g.dev_mode ? "左右调 上下移 OK切换" : "上下选 OK切换");
    canvas_draw_str_aligned(c, 126, 62, AlignRight, AlignBottom, "返回");
  } else {
    canvas_draw_str(c, 2, 62,
                    g.dev_mode ? "L/R:Adj U/D:Nav OK:Toggle"
                               : "U/D:Sel OK:Toggle");
...
static void draw_callback(Canvas *canvas, void *ctx) {
  ...
  if (g.mode == MODE_MENU) {
    ...
    const uint8_t *zh_items[M_COUNT] = {m1_bits, m2_bits, m3_bits,
                                        m4_bits, m5_bits, NULL};
    const char *en_items[M_COUNT] = {EN_M1,         EN_M2,        EN_M3,
                                     "4. Settings", "5. MC Beta", "6. Shop"};
    ...
    const char *zh_text_items[M_COUNT] = {NULL, NULL, NULL,
                                          NULL, NULL, "6. 商城"};
    // ---- 真正居中: 选中项永远在可见区正中央, 列表围绕它滚动 ----
    ...
    // 底部提示 (含语言切换说明)
    if (g.lang == LANG_ZH) {
      canvas_draw_xbm(canvas, 2, 62 - HINT_MENU_H + 1, HINT_MENU_W, HINT_MENU_H,
                      hint_menu_bits);
    } else {
  ...
  if (g.mode != MODE_MC) {
    ...
    if (g.stage == STAGE_COMBAT) {
      ...
      for (int i = 0; i < g.actor_count; i++)
        if (g.actors[i].active && g.actors[i].type == 0 && g.actors[i].hp > 0)
          alive++;
      // 弹药 (左中)
      char ab[12];
    ...
    uint16_t mc_mask =
        mc_day_masks[(g.cfg_mc_day_len < 4) ? g.cfg_mc_day_len : 0];
    ...
    // 成就统计
    char ach[24];
    snprintf(ach, sizeof(ach), "K%lu C%lu", g.ach_total_kills,
             g.ach_total_clears);
    ...
    if ((g.tick & 15) < 12) {
      const char *hint =
          (g.lang == LANG_ZH) ? "OK放 长OK挖 Back切" : "OK place OK-mine Back";
  ...
  if (g.show_hud) {
    ...
    if (g.mode == MODE_CAMPAIGN) {
      ...
      if (g.lang == LANG_ZH)
        canvas_draw_xbm(canvas, x + 7, 1, HUD_FLOOR_W, HUD_FLOOR_H,
                        hud_floor_bits);
      else
    ...
    if (g.stage == STAGE_PUZZLE || g.stage == STAGE_COMBAT) {
      ...
      if (g.lang == LANG_ZH)
        canvas_draw_xbm(canvas, x + 6, 1, HUD_TORCH_W, HUD_TORCH_H,
                        hud_torch_bits);
      else
  ...
  if (g.show_debug) {
    ...
    int lv = (g.mode == MODE_CAMPAIGN) ? g.level : g.endless_floor;
    snprintf(db, sizeof(db), "L%d X%d Y%d T%d A%d", lv, px, py, g.tick,
             g.actor_count);
  ...
  if (g.mode == MODE_LEVEL_CLEAR) {
    ...
    if (g.lang == LANG_ZH)
      ...
      snprintf(tb, sizeof(tb), "%s Score:%lu", EN_SHOP_TITLE,
    ...
    for (int i = 0; i < n; i++) {
      ...
      const char *name =
          (g.lang == LANG_ZH) ? shop_item_name_zh(i) : shop_item_name_en(i);
      const char *unit = (g.lang == LANG_ZH) ? "分" : "pts";
      char line[40];
      snprintf(line, sizeof(line), "%s  %d%s  x%d", name, shop_item_price(i),
               unit, g.items[i]);
    ...
    char tb[32];
    if (g.lang == LANG_ZH)
      snprintf(tb, sizeof(tb), "道具栏  HP:%d/%d  弹:%d", g.player.health,
               g.player.max_health, g.ammo);
    else
      snprintf(tb, sizeof(tb), "%s  HP:%d/%d  Ammo:%d", EN_SHOP_INV_TITLE,
               g.player.health, g.player.max_health, g.ammo);
    canvas_draw_str_aligned(canvas, 64, 1, AlignCenter, AlignTop, tb);
    canvas_draw_line(canvas, 0, 11, 127, 11);
    if (g.inv2_sel >= 10)
      g.inv2_sel = 0;
    ...
    for (int i = 0; i < n; i++) {
      ...
      const char *name =
          (g.lang == LANG_ZH) ? shop_item_name_zh(i) : shop_item_name_en(i);
      const char *desc =
          (g.lang == LANG_ZH) ? shop_item_desc_zh(i) : shop_item_desc_en(i);
      char line[40];
    ...
    if (g.buff_shield > 0 || g.buff_doublefire > 0) {
      char bf[40];
      if (g.lang == LANG_ZH)
        snprintf(bf, sizeof(bf), "护盾:%ds 火力:%ds", g.buff_shield / 60,
                 g.buff_doublefire / 60);
      else
        snprintf(bf, sizeof(bf), "Shield:%ds 2xFire:%ds", g.buff_shield / 60,
                 g.buff_doublefire / 60);
...
static void handle_overlay_input(InputKey key) {
  if (g.mode == MODE_LEVEL_CLEAR) {
    ...
    if (key == InputKeyUp) {
      ...
      g.mode =
          (s_resume_mode == MODE_LEVEL_CLEAR) ? MODE_LEVEL_CLEAR : MODE_MENU;
    }
  } else if (g.mode == MODE_SHOP_INV) {
    // v6.11: 新道具栏输入
    if (key == InputKeyUp) {
...
static void enter_level_select(bool for_campaign) {
  ...
  int default_endless =
      els[(g.cfg_endless_start < 5) ? g.cfg_endless_start : 0];
  g.ls_sel = for_campaign ? (g.campaign_cleared + 1) : default_endless;
  if (g.ls_sel < 1)
    g.ls_sel = 1;
  if (g.ls_sel > g.ls_max)
    g.ls_sel = g.ls_max;
  // 初始化滚动偏移: 让选中项尽量居中
  ...
  while (running) {
    ...
    if (st == FuriStatusOk && ev.type == EVENT_INPUT) {
      ...
      if (g.mode == MODE_OPENING) {
        ...
        did_input = true;
      } else if (g.mode == MODE_LEVEL_CLEAR || g.mode == MODE_GAME_OVER ||
                 g.mode == MODE_PAUSED) {
        if (type == InputTypeShort) {
        ...
        if (type == InputTypeShort) {
          ...
          if (g.mode == MODE_SHOP && key == InputKeyOk)
            sfx_play(SFX_MENU_OK);
          if (g.mode == MODE_SHOP_INV && key == InputKeyOk)
        ...
        if (g.mode == MODE_INVENTORY && key == InputKeyOk &&
            type == InputTypeLong) {
          ...
          GameMode mm = g.mode;
          if (mm == MODE_STORY && (key == InputKeyOk || key == InputKeyLeft ||
                                   key == InputKeyRight))
            old = SFX_STORY_TURN;
          if (mm == MODE_LEVEL_SELECT &&
              (key == InputKeyUp || key == InputKeyDown))
            old = SFX_MENU_MOVE;
          if (mm == MODE_LEVEL_SELECT && key == InputKeyOk)
            old = SFX_MENU_OK;
          if (mm == MODE_INVENTORY &&
              (key == InputKeyUp || key == InputKeyDown ||
               key == InputKeyLeft || key == InputKeyRight))
            old = SFX_MENU_MOVE;
          if (mm == MODE_INVENTORY && key == InputKeyOk)
            old = SFX_PICK_ITEM;
          if (mm == MODE_INVENTORY && key == InputKeyBack)
            old = SFX_MENU_OK;
          if (mm == MODE_MAP_PANEL)
            old = SFX_MENU_OK;
          if (old != SFX_NONE)
            sfx_play(old);
    ...
    bool in_game_view =
        (g.mode == MODE_CAMPAIGN || g.mode == MODE_ENDLESS_RUN ||
         ...
         g.mode == MODE_MC || g.mode == MODE_SHOP ||
         g.mode == MODE_SHOP_INV); // v6.11
