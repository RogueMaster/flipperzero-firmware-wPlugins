...
bool world_load(int level) {
  ...
  if (storage_file_open(f, path, FSAM_READ, FSOM_OPEN_EXISTING)) {
    if (storage_file_read(f, &wd, sizeof(wd)) == sizeof(wd) &&
        wd.magic == WORLD_MAGIC) {
      g.map_w = wd.map_w;
      g.map_h = wd.map_h;
      g.exit_x = wd.exit_x;
...
void storage_load(void) {
  ...
  if (ok && storage_file_read(f, &d, sizeof(d)) == sizeof(d) &&
    ...
    if (storage_file_open(f, SAVE_PATH, FSAM_READ, FSOM_OPEN_EXISTING) &&
        storage_file_read(f, &d4, sizeof(d4)) == sizeof(d4) &&
        d4.magic == SAVE_MAGIC_V4) {
      ...
      storage_file_close(f);
      if (storage_file_open(f, SAVE_PATH, FSAM_READ, FSOM_OPEN_EXISTING) &&
          storage_file_read(f, &magic_old, 4) == 4 &&
          magic_old == 0x4D415A33u) {
        int cc = 0, ef = 1;
