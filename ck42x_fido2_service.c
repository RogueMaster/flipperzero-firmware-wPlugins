...
#define CK_FIDO2_FILE_HEADER_SIZE \
    (CK_FIDO2_MAGIC_SIZE + CK_FIDO2_NONCE_SIZE + CK_FIDO2_TAG_SIZE)
...
static const uint8_t ck_fido2_magic[CK_FIDO2_MAGIC_SIZE] =
    {'C', 'K', 'F', '2', 'P', 'V', '1', 0};
...
static void ck_fido2_recover_interrupted_update(CkFido2Service* service) {
    ...
    FuriString* temporary = furi_string_alloc_printf(
        "%s%s", furi_string_get_cstr(path), CK_FIDO2_TEMP_SUFFIX);
    FuriString* backup = furi_string_alloc_printf(
        "%s%s", furi_string_get_cstr(path), CK_FIDO2_BACKUP_SUFFIX);
    bool target_exists =
        storage_common_stat(
            service->storage, furi_string_get_cstr(path), NULL) == FSE_OK;
    bool backup_exists =
        storage_common_stat(
            service->storage, furi_string_get_cstr(backup), NULL) == FSE_OK;
...
static bool ck_fido2_read_file(
    ...
    *output_length = 0;
    if(storage_file_open(file, furi_string_get_cstr(path), FSAM_READ, FSOM_OPEN_EXISTING)) {
        ...
        if(size <= capacity &&
           storage_file_read(file, output, (size_t)size) == (size_t)size) {
...
static CkFido2StoreState ck_fido2_decrypt_records(
    ...
    *count = 0;
    ...
    FuriString* path = ck_fido2_path(service);
    FS_Error stat = storage_common_stat(
        service->storage, furi_string_get_cstr(path), NULL);
    furi_string_free(path);
...
static bool ck_fido2_encrypt_records(
    ...
    if(storage_file_open(
           file, furi_string_get_cstr(temporary), FSAM_WRITE, FSOM_CREATE_ALWAYS)) {
    ...
    if(ok) {
        ...
        if(ok &&
           storage_common_rename(
               ...
               furi_string_get_cstr(path)) != FSE_OK) {
            ok = false;
            if(had_target)
                storage_common_rename(
                    service->storage,
...
static bool ck_fido2_report_is_cancel(
    const uint8_t report[CK_FIDO2_HID_REPORT_SIZE], uint32_t active_channel) {
    uint32_t channel = ((uint32_t)report[0] << 24) | ((uint32_t)report[1] << 16) |
                       ((uint32_t)report[2] << 8) | report[3];
    return channel == active_channel &&
           report[4] == (CK_FIDO2_HID_CMD_MASK | CK_FIDO2_HID_CMD_CANCEL) &&
           report[5] == 0U && report[6] == 0U;
}

static bool ck_fido2_consume_cancel(CkFido2Service* service) {
    if(furi_semaphore_acquire(service->request_semaphore, 0U) != FuriStatusOk)
        return false;
...
static bool ck_fido2_user_present(void* context) {
    ...
    while(service->running && elapsed < CK_FIDO2_SERVICE_APPROVAL_TIMEOUT_MS) {
        ...
        ck_fido2_send_message(
            service->active_channel,
            CK_FIDO2_HID_CMD_KEEPALIVE,
            &keepalive,
            sizeof(keepalive));
        FuriStatus status = furi_semaphore_acquire(
            service->approval_semaphore, CK_FIDO2_SERVICE_POLL_INTERVAL_MS);
...
static bool ck_fido2_load_credential(
    ...
    if(length) *length = 0;
    memset(records, 0, sizeof(records));
    if(!credential_id || !record || !length ||
       capacity < CK_CTAP2_CREDENTIAL_RECORD_SIZE)
        goto cleanup;
    if(ck_fido2_decrypt_records(service, records, &count) != CkFido2StoreValid)
        goto cleanup;
    for(size_t i = 0; i < count; i++) {
        if(memcmp(
               records[i] + CK_CTAP2_RECORD_CREDENTIAL_ID_OFFSET,
...
static int32_t ck_fido2_worker(void* context) {
    ...
    while(service->running) {
        if(service->deferred_count) {
            ...
            ck_fido2_zero(
                service->deferred_reports[service->deferred_head],
                CK_FIDO2_HID_REPORT_SIZE);
            service->deferred_head =
                (service->deferred_head + 1U) % CK_FIDO2_REQUEST_QUEUE_DEPTH;
            service->deferred_count--;
        } else {
            if(furi_semaphore_acquire(service->request_semaphore, FuriWaitForever) !=
               FuriStatusOk)
                continue;
        ...
        uint32_t offending_channel =
            ((uint32_t)report[0] << 24) | ((uint32_t)report[1] << 16) |
            ((uint32_t)report[2] << 8) | report[3];
        ...
        if(!complete) {
            if(error)
                ck_fido2_send_message(
                    offending_channel, CK_FIDO2_HID_CMD_ERROR, &error, 1U);
            continue;
        }
        ...
        if(ck_fido2_hid_dispatch(
               ...
               &response_command)) {
            ck_fido2_send_message(
                channel, response_command, response, response_length);
        }
...
CkFido2Service* ck_fido2_service_alloc(
    ...
    service->request_semaphore =
        furi_semaphore_alloc(CK_FIDO2_REQUEST_QUEUE_DEPTH, 0U);
    service->worker =
...
bool ck_fido2_service_start(CkFido2Service* service) {
    ...
    if(service->previous_usb != &usb_hid_u2f &&
       !furi_hal_usb_set_config(&usb_hid_u2f, NULL)) {
        if(service->previous_usb)
