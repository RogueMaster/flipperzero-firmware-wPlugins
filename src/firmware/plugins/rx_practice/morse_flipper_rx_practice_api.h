...
#define MORSE_FLIPPER_RX_PRACTICE_API_VERSION 11U
...
typedef struct {
    ...
    MfRxPracticeResult (*input)(void* state, const InputEvent* event, uint32_t now_ms);
    MfRxPracticeResult (*command)(void* state, MfRxPracticeCommand command, uint32_t now_ms);
    MfRxPracticeResult (
        *feed_text)(void* state, const char* text, size_t text_len, uint32_t now_ms);
} MfRxPracticeApi;
