...
bool http_get_http_response(char *buffer, size_t buffer_size)
{
    ...
    char file_path[256];
    snprintf(file_path, sizeof(file_path), STORAGE_EXT_PATH_PREFIX "/apps_data/%s/data/%s", "ghouls", last_saved_file_path);
    ...
    const char *payload)
{
    ...
    snprintf(flipperHttp->file_path, sizeof(flipperHttp->file_path), "%s/%s", STORAGE_EXT_PATH_PREFIX "/apps_data/ghouls/data", last_saved_file_path);
...
bool http_file_download(const char *url, const char *destination_path)
{
    ...
    flipperHttp->state = IDLE;
    if (!flipper_http_request(flipperHttp, BYTES, url, "{\"Content-Type\": \"application/octet-stream\"}", NULL))
    {
        return false;
    }
    flipperHttp->state = RECEIVING;
