...
The three concrete "fixup" changes made specifically to satisfy catalog build requirements and landing in this PR's referenced commit (`582ebeebadda8f9d225f38030f5d153051a55e63`) are:
...
Additional compliance fixes made to pass catalog lint/sanitizer requirements:

4. **README.md**: removed fenced code blocks and inline backticks (forbidden by the catalog Markdown sanitizer).
5. **CHANGELOG.md**: fixed `g.items[10]` syntax that triggered the Markdown `Short_reference` sanitizer failure.
6. **application.fam**: changed `fap_version` from tuple `(6,11,1)` to string `"6.11"` per Manifest.md "major.minor" format.
7. **All `.c`/`.h` files**: ran `clang-format` to fix formatting violations.
...
- **Manifest scope**: manifest points to upstream `k20120509/flipper-release@582ebeebadda8f9d225f38030f5d153051a55e63` via git source, pulls `description` from `@README.md`, `changelog` from `@CHANGELOG.md`, and 5 screenshots from `screenshots/ss0–4.png` at that upstream commit.
...
1. **Build validation** (already run locally against the official `tools/bundle.py` harness):
   ...
   ```
   Result: exit 0. Manifest loaded, upstream commit `582ebeebadda8f9d225f38030f5d153051a55e63` cloned and checked out successfully, FAM values (id=maze3d, name="Maze 3D", author=k20120509, category=Games) auto-populated correctly. See Author Checklist below.

2. **On-device playtest (human-performed, upstream commit `582ebeebadda8f9d225f38030f5d153051a55e63`)**:
...
- `applications/Games/maze3d/manifest.yml` — source=git, points to `k20120509/flipper-release` commit `582ebeebadda8f9d225f38030f5d153051a55e63`; references `@README.md`, `@CHANGELOG.md`, and 5 screenshots.
...
1. **PR body text drafting & English copy-editing**
   - Scope: the prose you are reading right now (Application Submission, Change Summary, Impact Scope, Testing Instructions, Extra Requirements, the Author Checklist wording, and this AI disclosure section) plus the English proofreading pass applied to the upstream `README.md` and `CHANGELOG.md` files at commit `582ebeebadda8f9d225f38030f5d153051a55e63`.
...
4. **clang-format compliance fixes**
   - Scope: running `clang-format -i *.c *.h` on all source files to pass catalog lint requirements.
   - What AI did: identified that clang-format violations were blocking the catalog lint step and suggested running clang-format on all files.
   - What human did: ran clang-format manually, verified the build still compiles correctly after reformatting, and confirmed all game logic was preserved unchanged (only whitespace/alignment adjustments).
