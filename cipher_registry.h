...
typedef CipherResult (*CipherTransformFn)(
    const char* input,
    int32_t key_a,
    int32_t key_b,
    const char* text_key);
...
typedef struct {
    ...
    uint8_t text_key_exact_len; // Exact length required for CipherKeyText, or 0 for unrestricted length
} CipherDef;
...
CipherResult cipher_registry_run(const CipherDef* def, bool is_decrypt, const char* input, int32_t key_a, int32_t key_b, const char* text_key);

// Writes <file_key>.txt or <file_key>_decrypt.txt into out. out must be at least 40 bytes.
void cipher_registry_build_filename(const CipherDef* def, bool is_decrypt, char* out, size_t out_size);