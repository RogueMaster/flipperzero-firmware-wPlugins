...
int bullets_active_count(void) {
    int n = 0;
    for(int i = 0; i < MAX_BULLETS; i++) if(g.bullets[i].active) n++;
    return n;
}
        ...
        if(a->cooldown > 0) { a->cooldown--; }
            ...
            if(chase) {
                ...
                if(fabsf(ddx) > fabsf(ddy)) {
                    dirs[0][0] = (ddx > 0) ? 1 : -1; dirs[0][1] = 0;
                    dirs[1][0] = 0; dirs[1][1] = (ddy > 0) ? 1 : -1;
                } else {
                    dirs[0][0] = 0; dirs[0][1] = (ddy > 0) ? 1 : -1;
                    dirs[1][0] = (ddx > 0) ? 1 : -1; dirs[1][1] = 0;
                }
                r = 0;
            }
            for(int k = 0; k < 4; k++) {
