...
static void quest_update(void) {
  ...
  for (int i = 0; i < g.quest.sub_count; i++) {
    ...
    if (s->target > 0 && s->progress >= s->target)
      s->done = true;
  }
  // 是否全部完成
  bool all = true;
...
static bool player_attack(void) {
  ...
  for (int i = 0; i < g.actor_count; i++) {
    Actor *a = &g.actors[i];
    if (!a->active || a->type != 0 || a->hp == 0)
      continue;
    float ex = a->x - px, ey = a->y - py;
    // 前方距离 (沿朝向投影)
    float fwd = ex * dx + ey * dy;
    if (fwd < 0.2f || fwd > 1.2f)
      continue;
    // 横向偏移 (垂直于朝向)
...
void player_shoot(void) {
  ...
  for (int i = 0; i < g.actor_count; i++) {
    ...
    if (fwd < 0.5f || fwd > 8.0f)
      continue; // 必须在前方 0.5~8 格
    float side =
        fabsf(ex * (-g.player.dir_y) + ey * g.player.dir_x); // 侧向偏移
    if (side > 2.0f)
      continue; // 视锥宽度 2 格
    // 综合评分: 距离越近 + 越居中 = 越优先
...
void ach_check(void) {
  if (g.ach_total_kills >= 1)
    ach_grant(ACH_FIRST_BLOOD, 0);
  if (g.ach_total_kills >= 10)
    ach_grant(ACH_KILL_10, 0);
  if (g.ach_total_kills >= 50)
    ach_grant(ACH_KILL_50, 0);
  if (g.ach_total_clears >= 1)
    ach_grant(ACH_FIRST_CLEAR, 0);
  if (g.ach_total_clears >= 10)
    ach_grant(ACH_CLEAR_10, 0);
  if (g.ach_total_clears >= 25)
    ach_grant(ACH_CLEAR_25, 0);
  if (g.ach_total_mined >= 50)
    ach_grant(ACH_MINER_50, 0);
  if (g.mode == MODE_CAMPAIGN) {
    if (g.level >= 21)
      ach_grant(ACH_REACH_COMBAT, 0);
    if (g.level >= 35)
      ach_grant(ACH_REACH_LATE, 0);
  }
}
...
static bool walkable(uint8_t c) {
  ...
  return c == CELL_EMPTY || c == CELL_KEY || c == CELL_EXIT ||
         c == CELL_TORCH || c == CELL_TRAP || c == CELL_POTION ||
         c == CELL_AMULET || c == CELL_LOCKED_EXIT;
}
static bool blocking(uint8_t c) {
  ...
  return c == WALL_BRICK || c == WALL_STONE || c == WALL_METAL ||
         ...
         c == WALL_WOOD || c == WALL_TREE || c == CELL_DOOR || c == WALL_SAND ||
         c == WALL_DIRT || c == WALL_LOG;
}
    ...
    for (int i = 0; i < MAX_PARTICLES; i++) {
      if (!g.particles[i].active) {
        ...
        if (type == 0) { // 射击火花: 朝前喷射, 短寿命, 很小
          ...
          ang +=
              ((float)(maze_rng_next() & 31) - 15.5f) * 0.04f; // ±0.6 rad 散射
...
bool player_move(float dx, float dy) {
  ...
  if (here == CELL_KEY) {
    ...
    if (g.stage == STAGE_MAZE_ONLY && g.mode == MODE_CAMPAIGN) {
      ...
      if (g.level > g.campaign_cleared)
        g.campaign_cleared = g.level;
      g.ach_total_clears++;
      ach_check();
      storage_save();
...
void actors_update(void) {
  for (int i = 0; i < g.actor_count; i++) {
    ...
    if (a->cooldown > 0) {
      ...
      if (a->type == 0) {
        ...
        if (dist2 < 64.0f) {
          ...
          for (float s = 0.5f; s < dist; s += 0.5f) {
            if (blocking(
                    maze_get((int)(a->x + nx * s), (int)(a->y + ny * s)))) {
              has_los = false;
              break;
            }
          }
      ...
      bool chase =
          (a->type == 0) && (has_los ? ((maze_rng_next() & 3) < 2) : true);
      if (chase) {
        float ddx = g.player.x - a->x, ddy = g.player.y - a->y;
...
void game_init_endless(int floor, bool visitor) {
  ...
  // v6.6: 先尝试加载缓存, 未命中则生成并保存
  unsigned seed = 0x12345678u ^ (unsigned)floor * 2654435761u ^
                  (unsigned)floor * 1013904242u;
...
void mc_cycle_block(void) {
  g.mc_block_type =
      (g.mc_block_type >= MC_BLOCK_COUNT) ? 1 : (g.mc_block_type + 1);
...
void game_init_mc(void) {
  ...
  g.mc_block_type =
      mc_default_sel[(g.cfg_mc_start_sel < 8) ? g.cfg_mc_start_sel : 0];
  g.mc_mined = 0;
  set_msg(MSG_MINE);
  g.dirty = true;
}
...
const char *shop_item_name_zh(int idx) {
  static const char *names[] = {
      "血量药水", "大血药", "满血药剂", "弹药+5",   "弹药满",
      "钥匙+1",   "火把+3", "护身符",   "无敌护盾", "双倍火力",
  };
  return (idx >= 0 && idx < 10) ? names[idx] : "?";
}
const char *shop_item_name_en(int idx) {
  static const char *names[] = {
      "Potion S", "Potion L", "Full Heal", "Ammo+5", "Ammo Max",
      "Key+1",    "Torch+3",  "Amulet",    "Shield", "2x Fire",
  };
  return (idx >= 0 && idx < 10) ? names[idx] : "?";
}
const char *shop_item_desc_zh(int idx) {
  static const char *desc[] = {
      "+5 HP",   "+10 HP",  "回满血", "+5 子弹", "子弹填满",
      "+1 钥匙", "+3 火把", "回起点", "5秒无敌", "15秒双倍火力",
  };
  return (idx >= 0 && idx < 10) ? desc[idx] : "";
}
const char *shop_item_desc_en(int idx) {
  static const char *desc[] = {
      "+5 HP",  "+10 HP",     "Full HP",    "+5 Bullets",    "Ammo Full",
      "+1 Key", "+3 Torches", "Warp Start", "Invincible 5s", "2x Fire 15s",
  };
  return (idx >= 0 && idx < 10) ? desc[idx] : "";
}
...
void game_handle_input(InputKey key, InputType type) {
  ...
  if (key == InputKeyUp || key == InputKeyDown) {
    ...
    static const float bak_ratio[] = {0.55f, 0.72f, 0.88f,
                                      1.00f}; // cfg_back_ratio
...
void game_update(void) {
  ...
  float JUMP_PEAK_LOCAL;
  {
    ...
    if (g.mode == MODE_MC && JUMP_PEAK_LOCAL > 3.0f)
      JUMP_PEAK_LOCAL = 3.0f;
  }
  ...
  if (regen_threshold < 5)
    regen_threshold = 5;
  if (regen_threshold > 240)
    regen_threshold = 240;
