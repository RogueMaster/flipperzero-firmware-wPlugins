...
bool zk_settings_load(ZkSettings* settings) {
    ...
    if(opened && storage_file_size(file) == sizeof(*settings)) {
        valid = storage_file_read(file, settings, sizeof(*settings)) == sizeof(*settings) &&
                settings->magic == ZK_SETTINGS_MAGIC &&
                settings->version == ZK_SETTINGS_VERSION && settings->default_name[0] &&
                settings->default_name[ZK_NAME_LENGTH - 1] == '\0';
    }
...
bool zk_settings_save(const ZkSettings* settings) {
    ...
    File* file = storage_file_alloc(storage);
    bool success =
        storage_file_open(file, ZK_SETTINGS_TEMP_PATH, FSAM_WRITE, FSOM_CREATE_ALWAYS);
    if(success) {
    ...
    if(success) {
        ...
        success = storage_common_rename(storage, ZK_SETTINGS_TEMP_PATH, ZK_SETTINGS_PATH) == FSE_OK;
    }
    furi_record_close(RECORD_STORAGE);
    return success;
}
