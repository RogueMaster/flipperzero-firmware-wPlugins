...
static void draw_chord_diagram(Canvas* canvas, int x, int y, const CuratedChord* chord) {
    ...
    for (int s = 0; s < 6; s++) {
        canvas_draw_line(canvas, x + s * string_spacing, y, x + s * string_spacing, y + num_frets * fret_height);
    }
    ...
    for (int s = 0; s < 6; s++) {
        ...
        if (val == -1) canvas_draw_str(canvas, sx, y - 3, "X");
        else if (val == 0) canvas_draw_str(canvas, sx, y - 3, "O");
    }
...
int32_t pocket_chords_app(void* p) {
    ...
    InputEvent event;
    while (app->running) {
        if (furi_message_queue_get(app->event_queue, &event, FuriWaitForever) != FuriStatusOk) continue;
        ...
        if (event.type == InputTypeShort || event.type == InputTypeRepeat) {
            ...
            if (event.key == InputKeyLeft || event.key == InputKeyRight) {
                ...
                while (new_index > 0 && chords[new_index - 1].name[0] == chords[new_index].name[0]) {
                    new_index--;
                }
                ...
                while (family_end < NUM_CHORDS - 1 && chords[family_end + 1].name[0] == current_root)
                    family_end++;
