...
int main(void) {
    ...
    /* ---- every real format the app writes --------------------------------- */
    {
        ...
        for(size_t i = 0; i < sizeof(real) / sizeof(real[0]); i++) {
            ...
            check(
                longest_line(buf) <= SPECTER_LOG_LINE_MAX, "a real entry stays inside the width");
    ...
    /* ---- edges ------------------------------------------------------------ */
    {
        ...
        check(
            longest_line(buf) <= SPECTER_LOG_LINE_MAX,
            "an over-long word is hard-split, not overflowed");
