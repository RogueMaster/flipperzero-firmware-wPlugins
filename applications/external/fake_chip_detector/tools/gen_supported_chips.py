...
# ---- IdCheck arrays -------------------------------------------------------
checks = {}
for m in re.finditer(r"static const IdCheck (\w+)\[\]\s*=\s*\{(.*?)\};", src, re.S):
...
# ---- chip table -----------------------------------------------------------
table = re.search(
    r"static const ChipEntry chip_db\[\]\s*=\s*\{(.*?)\n\};", src, re.S
).group(1)
    ...
    entries.append(
        {
            ...
            "hi": hi,
            "checks": (
                [] if m.group(6) == "NULL" else checks[m.group(6)][: int(m.group(7))]
            ),
            "note": note,
        }
    )
    ...
    for symbol, fields in re.findall(
        ...
        if chip and offer:
            defined[symbol] = {
                ...
                "file": mod.name,
                "addrs": (
                    [num(a) for a in addrs.group(1).split(",") if a.strip()]
                    if addrs
                    else None
                ),
            }
...
if missing:
    raise SystemExit(
        "registered in live_test.c but never defined: %s" % ", ".join(missing)
    )

orphans = sorted(set(defined) - registered)
if orphans:
    raise SystemExit(
        "defined but not registered in live_test.c: %s" % ", ".join(orphans)
    )
...
for chip_name, test in sorted(live_tests.items()):
    ...
    if test["addrs"] is None:
        addr_problems.append(
            "%s: live_test_%s has no .addrs" % (test["file"], test["slug"])
        )
        continue
    if entry["lo"]:
...
if addr_problems:
    raise SystemExit(
        "live-test addresses disagree with chip_db.c:\n  " + "\n  ".join(addr_problems)
    )
...
MODE_ALT = {
    "ModeAltSpi": "SPI",
    "ModeAltUart": "UART",
    "ModeAltOff": "nothing — it is held off",
}
mode_pins = []
...
families = re.findall(
    r'\{(0x[0-9A-Fa-f]+),\s*"([^"]*)",\s*"([^"]*)",\s*OneWireRole(\w+)\}', ow_body
)
...
generated = "\n".join(L)
summary = "%s: %d chips (%d with ID, %d address-only)" % (
    OUT,
    len(entries),
    len(ided),
    len(noid),
