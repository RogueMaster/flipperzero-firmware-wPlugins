...
enum {
    SR_GPS_FIRST_BLOCK_MS =
        8000, /* Soft timeout: changes wording only; sends no command, enters no terminal state */
    ...
    SR_GPS_STOP_ACK_MS =
        2000, /* Close-out confirmation window, same magnitude as SR_SCAN_CTL_TIMEOUT_MS */
};
 ...
 */
static inline SrGpsGate
    sr_gps_gate(bool link_ok, uint8_t session, uint8_t scan_ui, uint8_t phase) {
    if(!link_ok) {
