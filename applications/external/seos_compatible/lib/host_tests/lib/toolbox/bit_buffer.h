/* Host stand-in for the Flipper BitBuffer.
 *
 * Byte-addressed only; the app never needs sub-byte access. Unlike the
 * seader mock this one bounds-checks, because the overflows it would
 * otherwise hide are the ones the secure messaging tests are looking for.
 */
#pragma once
