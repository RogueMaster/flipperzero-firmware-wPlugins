...
static bool merge_ensure_cap(SubData* sd, size_t need) {
    if(need <= sd->cap) return true;

    if(need > MAX_SAMPLES) return false;

    size_t new_bytes = need * sizeof(int16_t);
    if(memmgr_get_free_heap() < new_bytes + LOAD_HEAP_RESERVE) return false;
...
static bool merge_append(SubData* dst, const SubData* src, bool add_separator) {
    ...
    if(!merge_ensure_cap(dst, dst->count + extra)) return false;
    if(add_separator) append_sample(dst, -MERGE_GAP_US);
...
static void run_merge(Storage* storage, DialogsApp* dialogs) {
    ...
    while(true) {
        ...
        dialog_file_browser_set_basic_options(&br, ".sub", &I_sub1_10px);
        br.base_path = SUBGHZ_DIR;
        if(!dialog_file_browser_show(dialogs, path, path, &br)) break;
        ...
        dialog_message_free(m);
        if(btn != DialogMessageButtonRight) break;
    }

    if(added > 0) recompute_total_us(&app->sd);
