...
static uint8_t ref_checksum(unsigned long long state) {
    ...
    for(uint8_t i = 0; i < 5; i++) {
        t >>= 8;
        sum = (uint8_t)(sum + reverse8((uint8_t)(t & 0xFF)));
    }
    sum = (uint8_t)(256 - sum);
    return reverse8(sum);
}
...
static int decode(const uint32_t* t, size_t n, unsigned long long* out) {
    ...
    for(int ph = 0; ph < 2; ph++) {
        if(t[i++] != 4480 || t[i++] != 4480) {
            printf("bad header ph%d\n", ph);
            return 0;
        }
        ...
        if(ph == 0) {
            if(t[i++] != 560 || t[i++] != 5600) {
                printf("bad inter-phase gap\n");
                return 0;
            }
        }
    }
    if(t[i++] != 560) {
        printf("bad trailing mark\n");
        return 0;
    }
    if(i != n) {
        printf("length %zu != %zu\n", i, n);
...
int main(void) {
    ...
    printf(
        "reset-state checksum: got %02X want %02X  %s\n", got, want, got == want ? "ok" : "FAIL");
    if(got != want) fails++;
    ...
    for(size_t k = 0; k < 4; k++) {
        if(!midea_ir_encode_toggle(&req, tog[k].tg, t, &n) || !decode(t, n, &v)) {
            fails++;
            continue;
        }
        int ok = v == tog[k].want;
    ...
    for(size_t k = 0; k < 4; k++) {
        if(!midea_ir_encode_extra(&req, ext[k].ex, t, &n) || !decode(t, n, &v)) {
            fails++;
            continue;
        }
        int ok = v == ext[k].want;
    ...
    for(int m = MideaModeCool; m < MideaModeCount; m++) {
        for(int f = 0; f < MideaFanCount; f++) {
            for(int temp = MIDEA_TEMP_MIN; temp <= MIDEA_TEMP_MAX; temp++) {
                ...
                if(!midea_ir_encode_state(&r, t, &n)) {
                    printf("encode fail\n");
                    fails++;
                    continue;
                }
                if(n != 199) {
                    printf("FAIL len %zu != 199\n", n);
                    fails++;
                    continue;
                }
                if(!decode(t, n, &v)) {
                ...
                if(((v >> 43) & 0x1F) != 0b10100) {
                    printf("FAIL header nibble\n");
                    fails++;
                }
                if(((v >> 40) & 0x07) != 0b001) {
                    printf("FAIL type field\n");
                    fails++;
                }
                if(((v >> 39) & 1) != 1) {
                    printf("FAIL power bit\n");
                    fails++;
                }
                if(((v >> 24) & 0x1F) != (unsigned)(temp - 17)) {
                    printf("FAIL temp %d\n", temp);
                    fails++;
                }
                if(((v >> 29) & 1) != 0) {
                    printf("FAIL fahrenheit set\n");
                    fails++;
                }
                if(((v >> 16) & 0xFF) != 0xFF) {
                    printf("FAIL byte2 not idle\n");
                    fails++;
                }
                if(((v >> 8) & 0xFF) != 0xFF) {
                    printf("FAIL byte1 not idle\n");
                    fails++;
                }
                if((v & 0xFF) != ref_checksum(v & ~0xFFULL)) {
    ...
    if(midea_ir_encode_toggle(&req, MideaTogglePowerOff, t, &n) && decode(t, n, &v)) {
        if(((v >> 39) & 1) != 0) {
            printf("FAIL power-off bit\n");
            fails++;
        }
        if((v & 0xFF) != ref_checksum(v & ~0xFFULL)) {
            printf("FAIL power-off checksum\n");
            fails++;
        }
