...
class HistoryChart(QWidget):
    ...
    def paintEvent(self, event) -> None:  # noqa: N802 — the name comes from Qt
        ...
        for i in range(5):
            ...
            pressure_label = (
                f"{p_lo + (p_hi - p_lo) * fraction:.{PRESSURE_UNITS[self._unit][1]}f}"
            )
            painter.drawText(
            ...
            temperature_label = (
                f"{t_lo + (t_hi - t_lo) * fraction:.{temperature_digits}f}"
            )
            painter.drawText(
        ...
        painter.drawText(
            header_rect, Qt.AlignLeft | Qt.AlignVCenter, self._title or "History"
        )
        ...
        temperature_rect = QRectF(
            header_rect.right() - temperature_width,
            header_rect.top(),
            temperature_width,
            header_rect.height(),
        )
        ...
        painter.drawText(
            pressure_rect, Qt.AlignRight | Qt.AlignVCenter, pressure_legend
        )
        painter.setPen(QPen(TEMPERATURE_COLOR, 1))
        painter.drawText(
            temperature_rect, Qt.AlignRight | Qt.AlignVCenter, temperature_legend
        )
