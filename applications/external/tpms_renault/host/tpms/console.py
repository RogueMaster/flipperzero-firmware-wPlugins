...
def _describe(reading: Reading) -> str:
    ...
    temperature = (
        "  -- " if frame.temperature_c is None else f"{frame.temperature_c:4d} C"
    )
    return (
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--port", help="Flipper serial port (found automatically by default)"
    )
    parser.add_argument(
        "--freq", type=int, default=DEFAULT_FREQUENCY, help="frequency in Hz"
    )
    ...
    if not port:
        ...
        if not ports:
            print(
                "No Flipper found. Connect it over USB or pass --port.", file=sys.stderr
            )
            return 2
        port = ports[0]
