...
static int check_vector(const TpmsTestVector* vector, int invert) {
    ...
    if(found->id != vector->id) {
        printf(
            "FAIL %-14s id %08lx, expected %08lx\n",
            vector->protocol,
            (unsigned long)found->id,
            (unsigned long)vector->id);
        failures++;
    }
    ...
    if(delta > 50 || delta < -50) {
        printf(
            "FAIL %-14s pressure %ld, expected %ld (hundredths of a kPa)\n",
            vector->protocol,
    ...
    if(vector->has_temperature && found->temperature_c != vector->temperature_c) {
        printf(
            "FAIL %-14s temperature %d, expected %d\n",
            vector->protocol,
            found->temperature_c,
    ...
    if(!failures && !invert) {
        printf(
            "ok   %-14s id=%0*lx  %ld.%02ld kPa  %d C\n",
            vector->protocol,
...
static const char* RENAULT_VECTOR = "01010101010101010110"
                                    "010110010110"
...
static int renault_hardware_cases(void) {
     ...
     * Manchester pairs come inverted. */
    {
        ...
        for(size_t i = 20; i < strlen(mixed); i++)
