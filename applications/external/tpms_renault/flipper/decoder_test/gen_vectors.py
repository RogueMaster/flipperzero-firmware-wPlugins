...
def schrader_smd3ma4_frame() -> list[str]:
    ...
    for check in range(4):
        ...
        for byte in b:
            total += (
                (byte & 3) + ((byte >> 2) & 3) + ((byte >> 4) & 3) + ((byte >> 6) & 3)
            )
        if (total & 3) == 1:
...
PROTOCOLS = [
    ...
    (
        ...
        180,
        50,
        "10100110101001100101101001011010",
        jansite_solar_frame,
        FILLER,
    ),
    (
        ...
        362,
        50,
        "10011001101010100101101001101010100110101010",
        jansite_ty588_frame,
        FILLER,
    ),
    ...
    ("nissan", 248, 120, "111101010101010101010101010101011110", nissan_frame, FILLER),
    ...
    (
        ...
        168,
        120,
        "1111010101010101010101010101010111",
        schrader_smd3ma4_frame,
        "",
    ),
    ...
    for name, number, chip_us, sync, builder, filler in PROTOCOLS:
        ...
        for payload in builder():
            ...
            if report:
                ...
                chips = (
                    stream
                    if SLICERS.get(name, "nrz") == "nrz"
                    else slicer_encode(stream)
                )
                chosen = (chips, report)
                break
        ...
        if not chosen:
            print(
                f"FAIL {name}: rtl_433 did not decode the generated frame",
                file=sys.stderr,
            )
            return 1
