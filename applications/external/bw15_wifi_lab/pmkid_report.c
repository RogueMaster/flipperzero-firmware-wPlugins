...
G5PmkidState g5_pmkid_classify_ok(
    const G5PmkidReport* report,
    const uint8_t requested_bssid[6],
    bool already_seen) {
...
size_t g5_pmkid_build_22000(const G5PmkidReport* report, char* out, size_t cap) {
    ...
    int n = snprintf(
        ...
        report->pmkid_hex,
        (unsigned)report->bssid[0],
        (unsigned)report->bssid[1],
        (unsigned)report->bssid[2],
        (unsigned)report->bssid[3],
        (unsigned)report->bssid[4],
        (unsigned)report->bssid[5],
        (unsigned)report->sta[0],
        (unsigned)report->sta[1],
        (unsigned)report->sta[2],
        (unsigned)report->sta[3],
        (unsigned)report->sta[4],
        (unsigned)report->sta[5],
        report->essid_hex);
