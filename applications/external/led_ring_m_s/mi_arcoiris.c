...
static void render_callback(Canvas* canvas, void* ctx) {
    ...
    // --- STATE 2: CONFIGURATION MENU ---
        ...
        if(app->config_index == 0)
            canvas_draw_str(canvas, 2, 26, "> Layout: ");
        else
            canvas_draw_str(canvas, 10, 26, "Layout: ");
        canvas_draw_str(
            canvas,
            60,
            26,
            (app->tipo_led == TypeRing) ? "< Circular Ring >" : "< Square Matrix >");

        if(app->config_index == 1)
            canvas_draw_str(canvas, 2, 39, "> LED Count: ");
        else
            canvas_draw_str(canvas, 10, 39, "LED Count: ");
        char buf_count[16];
    ...
    // --- STATE 3: LIVE EFFECTS GENERATOR ---
        ...
        case ModoArcoiris:
            canvas_draw_str(canvas, 2, 23, "Mode: Rainbow Spectrum");
            break;
        case ModoPulso:
            canvas_draw_str(canvas, 2, 23, "Mode: Color Breath");
            break;
        case ModoKitt:
            canvas_draw_str(canvas, 2, 23, "Mode: Knight Rider KITT");
            break;
        case ModoFuego:
            canvas_draw_str(canvas, 2, 23, "Mode: Organic Fire");
            break;
        default:
            break;
        }
        ...
        snprintf(
            ...
            "Pwr: %d%% | Spd: %dx | LEDs: %d",
            app->brillo_porcentaje,
            app->velocidad_factor,
            app->total_leds);
...
int32_t arcoiris_main(void* p) {
    ...
    while(app->ejecutando) {
                ...
                // BUTTONS LOGIC: CONFIGURATION MENU
                    ...
                    if(evento.key == InputKeyBack) {
                        ...
                        if(evento.key == InputKeyRight && app->total_leds < LED_MAX_COUNT) {
                            app->total_leds = (app->total_leds == 16) ? 24 :
                                              (app->total_leds == 24) ? 32 :
                                              (app->total_leds == 32) ? 64 :
                                                                        app->total_leds + 1;
                        } else if(evento.key == InputKeyLeft && app->total_leds > LED_MIN_COUNT) {
                            app->total_leds = (app->total_leds == 64) ? 32 :
                                              (app->total_leds == 32) ? 24 :
                                              (app->total_leds == 24) ? 16 :
                                                                        app->total_leds - 1;
                        }
                    }
                }
        ...
        if(app->modo_actual == ModoArcoiris) {
            ...
            if(posicion_activa >= app->total_leds)
                posicion_activa = (app->total_leds * 2 - 1) - posicion_activa;
            ...
            for(int i = 0; i < app->total_leds; i++) {
                if(i == posicion_activa) {
                    buffer_leds[i * 3 + 1] = r_k;
                    buffer_leds[i * 3] = g_k;
                    buffer_leds[i * 3 + 2] = b_k;
                } else if(i == posicion_activa - 1 || i == posicion_activa + 1) {
                    buffer_leds[i * 3 + 1] = r_k / 4;
                    buffer_leds[i * 3] = g_k / 4;
                    buffer_leds[i * 3 + 2] = b_k / 4;
                }
            }
        } else if(app->modo_actual == ModoFuego) {
            for(int i = 0; i < app->total_leds; i++) {
                uint32_t rnd = furi_hal_random_get();
                uint8_t compensacion_matriz =
                    (app->tipo_led == TypeMatrix && i < app->total_leds / 4) ? 40 : 0;
