#include "nfc_tools_mime_input.h"
...
//   7  8  9  →  /  .  -   (useful for MIME types, e.g. text/plain)
...
static const MimeInputKey keyboard_keys_row_1[] = {
    {'q', 1, 8},
    {'w', 10, 8},
    {'e', 19, 8},
    {'r', 28, 8},
...
static const MimeInputKey keyboard_keys_row_2[] = {
    {'a', 1, 20},
    {'s', 10, 20},
    {'d', 19, 20},
    {'f', 28, 20},
...
static const MimeInputKey keyboard_keys_row_3[] = {
    {'z', 1, 32},
    {'x', 10, 32},
    {'c', 19, 32},
    {'v', 28, 32},
...
static const MimeInputKey* get_row(uint8_t row_index) {
    switch(row_index + 1) {
    case 1:
...
static char get_selected_char(MimeInputModel* model) {
...
static void mime_input_view_draw_callback(Canvas* canvas, void* _model) {
    MimeInputModel* model = _model;
    ...
    for(uint8_t row = 0; row < keyboard_row_count; row++) {
        ...
        const MimeInputKey* keys = get_row(row);
...
static void mime_input_backspace(MimeInputModel* model) {
    uint8_t len = model->clear_default_text ? 1 : (uint8_t)strlen(model->text_buffer);
    if(len > 0) model->text_buffer[len - 1] = '\0';
}

static void mime_input_handle_ok(MimeInputModel* model, bool shift) {
...
static bool mime_input_view_input_callback(InputEvent* event, void* context) {
    MimeInput* mi = context;
    bool consumed = false;

    MimeInputModel* model = view_get_model(mi->view);
       ...
       event->type == InputTypeRepeat) {
        ...
        case InputKeyOk:
            if(event->type != InputTypeRepeat)
                mime_input_handle_ok(model, event->type == InputTypeLong);
            break;
        case InputKeyBack:
    ...
    view_commit_model(mi->view, consumed);
    return consumed;
}
...
MimeInput* mime_input_alloc(void) {
    MimeInput* mi = malloc(sizeof(MimeInput));
    mi->view = view_alloc();
    view_set_context(mi->view, mi);
    view_allocate_model(mi->view, ViewModelTypeLocking, sizeof(MimeInputModel));
    view_set_draw_callback(mi->view, mime_input_view_draw_callback);
    view_set_input_callback(mi->view, mime_input_view_input_callback);
    mime_input_reset(mi);
    return mi;
}

void mime_input_free(MimeInput* mi) {
    furi_check(mi);
    view_free(mi->view);
    free(mi);
}

void mime_input_reset(MimeInput* mi) {
    furi_check(mi);
    with_view_model(
        mi->view,
        MimeInputModel * model,
        {
            model->header = "";
...
View* mime_input_get_view(MimeInput* mi) {
    furi_check(mi);
    return mi->view;
}

void mime_input_set_header_text(MimeInput* mi, const char* text) {
    furi_check(mi);
    with_view_model(mi->view, MimeInputModel * model, { model->header = text; }, true);
}
...
void mime_input_set_minimum_length(MimeInput* mi, size_t minimum_length) {
    with_view_model(
        mi->view, MimeInputModel * model, { model->minimum_length = minimum_length; }, true);
}
