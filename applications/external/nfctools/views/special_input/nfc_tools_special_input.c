...
static uint8_t get_row_size(uint8_t row_index) {
    ...
    switch(row_index + 1) {
    case 1:
        return COUNT_OF(si_abc_row_0); // 14
    case 2:
        return COUNT_OF(si_abc_row_1); // 13
    case 3:
        return COUNT_OF(si_abc_row_2); // 12
    default:
        furi_crash();
    }
}
...
static void draw_page_key(Canvas* canvas, uint8_t x, uint8_t y, bool selected, uint8_t page) {
    canvas_set_color(canvas, ColorBlack);
    if(selected) {
...
void special_input_set_minimum_length(SpecialInput* si, size_t minimum_length) {
    with_view_model(
        si->view, SpecialInputModel * model, { model->minimum_length = minimum_length; }, true);
}
