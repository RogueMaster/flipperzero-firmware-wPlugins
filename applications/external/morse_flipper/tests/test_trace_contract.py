...
CONTENT = (ROOT / "src/firmware/morse_flipper_content_host.c").read_text(
    encoding="utf-8"
)
...
def main() -> None:
    ...
    open_trace = trace.index("morse_flipper_scene_open(app, MorseFlipperSceneTrace)")
    ...
    assert (
        "morse_flipper_handle_active_keying_event(app, event);" in route[trace_route:]
    )

    keying = function_body(INPUT, "morse_flipper_handle_active_keying_event", "void ")
    ...
    trace_view = VIEW[
        VIEW.index("if(app->screen == MorseFlipperScreenTrace)") : VIEW.index(
