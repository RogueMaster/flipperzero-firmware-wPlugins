...
def read_manifest_version() -> str:
    ...
    if len(active) != 1:
        fail(
            f"application.fam must contain exactly one active fap_version, found {len(active)}"
        )
...
if manifest_version != EXPECTED_VERSION:
    fail(
        f"application.fam fap_version is {manifest_version!r}, expected {EXPECTED_VERSION!r}"
    )
