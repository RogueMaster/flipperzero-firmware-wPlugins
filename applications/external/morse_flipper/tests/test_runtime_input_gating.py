...
def main() -> None:
    ...
    assert (
        "morse_keyer_paddle_event(&app->keyer, paddle, after != 0U, now_ms);" in shared
    )
    ...
    drop = transition.index(
        "morse_flipper_drop_live_keying_for_playback(app, now_ms);", drain
    )
