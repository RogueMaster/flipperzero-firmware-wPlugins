...
assert scenes.index("morse_flipper_radio_host_close") < scenes.index(
    "morse_flipper_ardf_host_open"
)
...
assert (
    "assets_icons.h"
    not in (root / "src/firmware/plugins/ardf/mf_ardf_draw.c").read_text()
)
assert host.index("morse_flipper_plugin_runtime_apply_result_locked") < host.index(
    "furi_mutex_release"
)
...
assert host.index("furi_mutex_release") < host.rindex(
    "morse_flipper_ardf_host_apply_after_unlock"
)
...
audio_pwm_scenes = audio_route.split("bool morse_flipper_scene_supports_audio_pwm", 1)[
    1
].split("bool morse_flipper_audio_path_is_sampled", 1)[0]
