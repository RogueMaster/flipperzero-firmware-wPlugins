...
TYPES = (
    ROOT / "src/firmware/plugins/rx_practice/morse_flipper_rx_practice_types.h"
).read_text()
API = (
    ROOT / "src/firmware/plugins/rx_practice/morse_flipper_rx_practice_api.h"
).read_text()
...
def main() -> None:
    ...
    rx_live_reject = INPUT.index(
        "app->screen == MorseFlipperScreenRxPractice && !g.live"
    )
    assert (
        "event->key == InputKeyBack && g.back_key"
        not in INPUT[
            INPUT.index(
                "static bool\n    morse_flipper_session_live_keying_input"
            ) : rx_live_reject
        ]
    )
    ...
    assert (
        ...
        in (ROOT / "src/firmware/morse_flipper_live_view_common.c").read_text()
    )
