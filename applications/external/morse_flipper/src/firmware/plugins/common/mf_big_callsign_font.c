...
void mf_big_callsign_draw_char(Canvas* canvas, int32_t x, int32_t y, char ch, bool inverted) {
    ...
    if(inverted) {
        canvas_draw_box(
            canvas, x - 1, y - 1, MF_BIG_CALLSIGN_WIDTH + 2U, MF_BIG_CALLSIGN_HEIGHT + 2U);
...
void mf_big_callsign_draw_text(
    ...
    left = (128 - ((int32_t)slots * MF_BIG_CALLSIGN_WIDTH +
           ...
           2;
    while(count < slots && text[count] != '\0')
        count++;
    for(uint8_t i = 0U; i < slots; i++) {
        bool mismatch = mark_errors && (i >= count || target == NULL || text[i] != target[i]);
