...
def clean_source_pcm(raw: bytes) -> tuple[array[int], dict[str, int | float]]:
    ...
    normalised = array(
        "h",
        (max(-32768, min(32767, round(value * gain))) for value in trimmed),
    )
...
def synthesize(
    client: Any, spoken_text: str, rate: str | None
...
def encode_variant(
    source: Path, destination: Path, sample_rate: int, codec: str
) -> None:
    destination.parent.mkdir(parents=True, exist_ok=True)
    filters: list[str] = []
    ...
    for rate_name in RATES:
        for token_name, spoken_text in TOKENS.items():
            ...
            for variant_name in VARIANTS:
                relative = (
                    Path("audition") / rate_name / variant_name / f"{token_name}.wav"
                )
                cells.append(
                    "<td>"
...
def main() -> int:
    ...
    if (
        args.runtime_pack is not None
        and args.mastering_profile != "passive-presence-gate"
    ):
        raise ValueError(
            "--runtime-pack requires --mastering-profile passive-presence-gate"
        )
    ...
    for rate_name, rate in RATES.items():
        for token_name, spoken_text in TOKENS.items():
            ...
            if not args.encode_only and (args.force or not source_path.exists()):
                ...
                raise RuntimeError(
                    f"source WAV is missing for --encode-only: {source_path}"
                )
            ...
            for variant_name, (sample_rate, codec) in VARIANTS.items():
                encoded_path = (
                    output / "encoded" / rate_name / variant_name / f"{token_name}.wav"
                )
                audition_path = (
                    output / "audition" / rate_name / variant_name / f"{token_name}.wav"
                )
                ...
                logical_samples = round(
                    len(reference_samples) * sample_rate / SOURCE_RATE
                )
    ...
    for rate_name in RATES:
        ...
        for variant_name in VARIANTS:
            entries = [
                manifest["tokens"][f"{rate_name}/{token_name}"]["variants"][
                    variant_name
                ]
                for token_name in TOKENS
            ]
    ...
    if args.runtime_pack is not None:
        manifest["runtime_pack"] = build_runtime_pack(
            output, args.runtime_pack, decoded
        )
