...
TOKENS = tuple("ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789") + (
    "stroke",
    "period",
    "comma",
    "question-mark",
...
def parse_pcm16_16k_pack(blob: bytes) -> tuple[list[tuple[int, bytes, int]], int]:
    ...
    if (
        file_size != len(blob)
        or table_offset < HEADER.size
        or table_end > data_offset
    ...
    for index in range(count):
        token_id, offset, length, samples, predictor, ima_index, reserved = (
            ENTRY.unpack_from(table, index * ENTRY.size)
        )
...
def read_pcm16_wavs(directory: Path) -> list[tuple[int, bytes, int]]:
    """Read the canonical mono PCM16/16 kHz token WAVs in token-ID order."""
    payloads = []
    for token_id, token in enumerate(TOKENS):
        path = directory / f"{token}.wav"
        with wave.open(str(path), "rb") as source:
            if (
                ...
                raise ValueError(f"{path} is not mono PCM16/16 kHz WAV")
            samples = source.getnframes()
            payload = source.readframes(samples)
        if samples == 0 or len(payload) != samples * 2:
            raise ValueError(f"invalid token WAV {path}")
        payloads.append((token_id, payload, samples))
    return payloads


def write_pack(
    payloads: list[tuple[int, bytes, int]], codec: int, rate: int, output: Path
) -> None:
    ...
    output.write_bytes(header + table + data)
...
def convert_pcm16_16k_to_u8(source: Path, output: Path) -> None:
    ...
    payloads = [
        (token_id, pcm16_to_u8(payload), samples)
        for token_id, payload, samples in tokens
    ]
    table_offset = HEADER.size
    data_offset = table_offset + ENTRY.size * len(payloads)
    table = bytearray()
...
def main() -> int:
    ...
    parser.add_argument(
        "--input", type=Path, help="directory containing token-id .bin payloads"
    )
    parser.add_argument(
        "--wav-input",
        type=Path,
        help="directory containing mono PCM16/16 kHz token WAVs",
    )
    parser.add_argument("--variant", choices=CODECS)
    parser.add_argument(
        "--from-mfva", type=Path, help="selected PCM16/16 kHz MFVA input to convert"
    )
    parser.add_argument("--output", type=Path, required=True)
    parser.add_argument(
        "--samples", type=Path, help="JSON-like line file: token-id sample-count"
    )
    args = parser.parse_args()
    if args.from_mfva is not None:
        if (
            args.input is not None
            or args.wav_input is not None
        ...
        return 0
    if args.wav_input is not None:
        if (
            args.input is not None
            or args.samples is not None
            or args.variant != "s16_16k"
        ):
            parser.error(
                "--wav-input requires --variant s16_16k and cannot use --input or --samples"
            )
        write_pack(read_pcm16_wavs(args.wav_input), *CODECS[args.variant], args.output)
        return 0
    if args.input is None or args.variant is None or args.samples is None:
        parser.error(
            "--input, --variant, and --samples are required for a binary payload build"
        )
    ...
    for token_id, token in enumerate(TOKENS):
        ...
        expected = (
            samples * 2 if codec == 0 else (samples + 1) // 2 if codec == 3 else samples
        )
        if len(data) != expected:
            raise ValueError(
                f"encoded length mismatch for {token}: {len(data)} != {expected}"
            )
        payloads.append((token_id, data, samples))
    write_pack(payloads, codec, rate, args.output)
    return 0
