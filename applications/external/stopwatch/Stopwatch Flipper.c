...
static void render_callback(Canvas* canvas, void* ctx) {
    ...
    for(int i = start_idx; i < model->split_count; i++) {
        ...
        if(s_sec >= 60) {
            snprintf(split_str, sizeof(split_str), "%d) %lu:%02lu.%02lu", i + 1, s_sec / 60, s_sec % 60, s_cs);
        } else {
                    ...
                    switch(event.input.key) {
                        ...
                        case InputKeyDown:
                            ...
                            if(app->model->running && app->model->split_count < MAX_SPLITS) {
                                uint32_t current_ms = app->model->elapsed_ms + (furi_get_tick() - app->model->start_tick);
