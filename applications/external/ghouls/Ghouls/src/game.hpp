...
private:
    ...
    Sprite3D* wallSprite =
        nullptr; // Static Sprite3D instance for horizontal walls (top/bottom borders)
    Sprite3D* vWallSprite =
        nullptr; // Static Sprite3D instance for vertical walls (left/right borders)
    //
    static const Vector
        housePositions[HOUSE_SPAWN_COUNT]; // pre-computed house spawn positions (scaled)
    static const Vector
        treePositions[TREE_SPAWN_COUNT]; // pre-computed tree spawn positions (scaled)
    static const Vector
        wallPositions[MAP_OUTER_WALLS]; // pre-computed wall center positions (for collision)
    static const Vector wallSegmentPositions
        [WALL_SEGMENT_COUNT]; // pre-computed wall segment positions (for rendering)
    //
    ...
    bool
        initializeSprites(); // initialize the static Sprite3D instances for trees, houses, and walls
    ...
    void
        registerSpritePositionsOnMap(); // register the positions of the static sprites (houses, trees, walls) on the DynamicMap for collision detection
    bool removeGhoulsFromLevel(); // remove all ghouls from the level
    void renderWalls(
        Game* game); // render walls in the level based on their positions relative to the player
...
public:
    ...
    //
    bool collisionMapCheck(
        Vector new_position); // check if the new position collides with any tiles on the map
    ...
    DynamicMap* getCurrentDynamicMap() const {
        return currentDynamicMap;
    } // Get the current dynamic map
    bool
    ...
    void renderEnvironment(
        Game* game); // Render the environment (houses, trees, walls) in the level based on their positions relative to the player
    void resetInput() {
        lastInput = -1;
