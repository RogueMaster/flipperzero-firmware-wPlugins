## 2.16-beta.29 — web flasher fix, LilyGO deep-sleep bus release

- **The browser flasher can download firmware now (#176).** It handed ESP Web Tools the GitHub release download URLs, and GitHub sends no CORS headers on those (neither the redirect nor the asset host), so the browser fetch failed with "can't fetch" on the hosted page and in the downloaded `tesla-flasher.html`. A new `pages` workflow publishes the newest release's `-merged.bin` images to GitHub Pages under `/firmware/` after every tagged build (Pages sends `Access-Control-Allow-Origin: *`), and the flasher downloads from there. Docs pages keep building with Jekyll as before. Thanks @adrianpadure99 for the report. (PR #189)
- **LilyGO: the CAN bus is released before deep sleep.** The T-CAN485 went to sleep with TWAI still installed and nothing holding the TX pad, so a floating TXD could pull the bus dominant (and re-trigger the wake on RX). It now stops the controller, holds TX recessive through sleep with the RTC pad hold, and releases the hold on every boot before TWAI starts. Same class of fix as the pre-reboot quiesce in beta.28 (#180). (PR #188)

## 2.16-beta.28 — OTA false-latch fix, HW4 AP-state decode, Autopark pause

- **ESP32: a false "OTA in progress" no longer silently stops all TX (#183).** On current cars `0x318` byte6 is a rolling counter that steps by 2, not the `GTW_updateInProgress` flag, so the two bits we read just alternate 1/3. The ESP32 build treated 1 as "installing": a few dropped RX frames (normal on a busy bus) gave three 1s in a row, OTA latched, `fsd_can_transmit()` refused everything, and it could never clear because clearing needed six non-1 samples in a row. The Flipper core had used 2 since #19, so the two builds disagreed. Both now share one header-only decision (`fsd_logic/fsd_ota.h`): only a *stable* raw 2 counts (byte6 identical to the previous frame), 3 such samples in a row latch (4 identical frames), 6 of anything else release. (PR #184) If your nag killer "did nothing" on an ESP32 build, re-test on this one. Thanks @Tesla234987234sdf (report + the Palladium capture that showed the counter), @anoblekman (the same finding on a Highland, #177), and @ukinora, who found and fixed it independently in their fork.
- **HW4: `DAS_autopilotState` is read from `0x39B` byte0 low nibble, the DBC position (#177).** The old decode read byte1[7:4], which is the top bit of `DAS_fusedSpeedLimit` plus three summon/speed-warning flags, never the AP state. Every car we have data for carries it in byte0 (#116, #163, #177), and ev-open-can-tools dropped the byte1 read in July. The #116 byte0 latch is gone with it: it was disqualified on the first frame whenever the car booted under an 80 km/h limit, which is why it never engaged on @anoblekman's car (or on-car in beta.20). AP status, AP-First, Soft Engage and Abort Guard now see the real state on Highland / Juniper. Signal Map workarounds for this can be cleared. Thanks @anoblekman. (PR #186)
- **All TX pauses during in-car Autopark (#180).** Highland runs in-car Autopark at DAS state 6; injecting during it threw AEB / traction / stability warnings. An Autopark episode starts when the car enters state 6 straight from ≤2 (FSD passes through 3) or with `DAS_autoparkReady` / `autoParked` / `autoparkWaitingForBrake` set (at entry, in the 3 s before, or during), and ends when the state leaves 6. It releases above 20 km/h (fresh, valid vehicle speed; an SNA speed frame counts as unknown), so FSD engaged in a car park can't keep the nag killer off on the road. The ESP32 now parses `0x257` DI_speed for this. Enforced in `fsd_can_transmit()`, so it covers every TX path. Not a blanket state-6 block — 6 is also normal FSD engaged. Thanks @anoblekman. (PR #186)
- **"Engaged" means DAS state 3–6 everywhere.** The ESP32 HW3 path counted only 3 as engaged, so a car whose steady state is 6 read as "AP off" (#108). (PR #186)
- **Signal Map hardening (#100).** A field with mask 0 is ignored instead of forcing it to 0, and the dashboard warns when the configured DAS id never shows up on the tapped bus (the nag killer stays paused in that case, as before, but now it says why). Found from @tommybsb-lab's screenshots. (PR #186)
- **ESP32: CAN controllers are stopped before every reboot (#180).** Factory reset, WiFi apply, restart and post-OTA-flash all quiesce the controllers first (TWAI stop + uninstall with TX held recessive, MCP2515 to config mode). (PR #185)
- **ESP32 dashboard: "TX Errors" is now "CAN Errors", with an RX-drop / bus / TX-fail breakdown.** The old tile mostly counted RX-queue drops, which read like failed transmissions. (PR #185)
- **Tests:** the ESP32 handlers are now built and unit-tested on the host too (`test/test_esp32_core.cpp`). Host tests only compiled the Flipper core before, which is how the OTA mismatch went unnoticed.
- **Flipper app catalog:** `.catalog/README.md` + `.catalog/changelog.md` in the catalog's restricted markdown, for the catalog submission.

**Heads-up for HW4 + FSD unlock + AP-First on 2026.14 and newer:** AP-First was effectively stuck closed on HW4 (the byte1 decode never reported "engaged"), so it never injected. It now opens after engage as designed, and on 14.x+ firmware injection after engage can make the car drop AP about a second later (the preflight, see #122 / #168). Keep FSD unlock off on 14.x+.

## 2.16-beta.27 — capture fidelity, Flipper Signal Map, drive-gear safety

- **Capture fidelity — web-stream captures now self-label as full-rate or lossy.** A new controller-level `rx_missed` metric (the real silent-decimation source on a busy all-ID capture) is exposed alongside the existing stream-ring `dropped`, in the dashboard JSON and — opt-in via `?meta=1` — as `#`-prefixed header/footer comment lines on the stream itself (`ids`, `bus`, `mode=single-id-hwfilter|all-id-decimated`, `rx_missed`). The default stream body is byte-identical, so existing consumers are unaffected. (PR #169)
- **Flipper "Signal Map" preset picker.** New Settings item to relocate where the nag / AP logic reads `DAS_autopilotState` + `DAS_handsOnState` for cars whose `0x39B` / `0x399` layout is non-standard: Auto / `0x39B` b0 (HW4) / `0x39B` b1 (HW4) / `0x399` b0 (HW3). The configurable-mapping mechanism has been in the shared core since #122 (and on the ESP32 dashboard); this exposes it on the Flipper and wires the previously-uncalled `fsd_apply_signal_config()` into the Flipper worker. Default Auto, so behaviour is unchanged unless a preset is chosen. (PR #171)
- **Summon EU Unlock auto-disables on drive-gear engage.** When the gear lever is pulled into a drive gear (`0x229` full-down), Summon EU Unlock switches itself off (one-shot, persisted) on both HW3 and HW4 — a safety guard so the distance unlock can't linger while driving. (PR #165, from @densen2014's suggestion in #160)
- **TLSSC bit38 toggle on the ESP32.** Adds the missing runtime toggle for the `0x3FD` mux0 bit38 TLSSC assist flag. (PR #159, @densen2014)
- **Build: all ESP32 envs pinned to `espressif32@6.9.0`.** Prevents Arduino-ESP32 3.x from resolving in and overflowing DRAM on the classic-ESP32 targets. (PR #167)
- **Docs:** pinned [FSD-on-14.x FAQ (#168)](https://github.com/hypery11/flipper-tesla-fsd/discussions/168) — the canonical answer to "does this unlock FSD on 2026.14.x / .20 / .26.x?" — now linked from all three READMEs. (PR #170)
