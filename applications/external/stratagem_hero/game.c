...
static void
    game_widget_navigate(StrataHeroGameWidget* widget, StrataHeroGameWidgetNavigationEvent event);
...
static void next_code(StrataHeroGameModel* model) {
    ...
    model->current_code_length =
        strlen(model->round_stratagems[model->current_round_stratagem]->code);
...
static void draw_gameplay(Canvas* canvas, StrataHeroGameModel* model) {
    ...
    if(model->score_popup_points > 0) {
        ...
        canvas_draw_str_aligned(
            canvas, SCREEN_WIDTH, SCREEN_HEIGHT - 12, AlignRight, AlignBottom, buffer);
    }
    ...
    for(int i = 0; i < code_len; i++) {
        ...
        const Icon* icon = inverse ?
                               glyph->inverse :
                               ((i < model->current_code_progress) ? glyph->black : glyph->white);
        ...
        if(event->type == InputTypeShort) {
            ...
            if(code_input != 0) {
                const Stratagem* stratagem =
                    model->round_stratagems[model->current_round_stratagem];
                if(stratagem->code[model->current_code_progress] == code_input) {
                    ...
                    if(last) {
                        stratahero_code_complete_notification(
                            widget->notification, &widget->settings);
                    } else {
                        stratahero_code_glyph_entry_success_notification(
                            widget->notification, &widget->settings);
                    }
                    with_widget_model(
                        widget,
                    ...
                    stratahero_code_glyph_entry_failure_notification(
                        widget->notification, &widget->settings);
    ...
    case GameView_QuitConfirmation: {
        if(event->type == InputTypeShort) {
            if(event->key == InputKeyLeft || event->key == InputKeyBack) {
                ...
                with_widget_model(widget, m, { m->current_view = GameView_GameOver; }, true);
                furi_timer_start(widget->gameover_timer, GAMEOVER_DELAY);
                handled = true;
            }
        }
...
StrataHeroGameWidget* stratahero_game_widget_alloc() {
    ...
    widget->gameplay_timer =
        furi_timer_alloc(gameplay_timer_callback, FuriTimerTypePeriodic, widget);
    widget->gameover_timer = furi_timer_alloc(gameover_timer_callback, FuriTimerTypeOnce, widget);
    widget->invalid_code_timer =
        furi_timer_alloc(invalid_code_timer_callback, FuriTimerTypeOnce, widget);
    widget->score_popup_timer =
        furi_timer_alloc(score_popup_timer_callback, FuriTimerTypeOnce, widget);
