...
- **Level progression:** standard-array defaults, explicit ASI/Feat level choices, deterministic class/subclass/species progression and **Grant Initial Traits**. Level Choices always opens and reports when no ASI/Feat choice is pending. Grant Initial Traits immediately applies deterministic starting grants; spell grants verify the Spellbook sidecar so a stale applied marker can repair a missing deterministic species spell without duplicating an existing spell. A bounded post-level **Level-Up Review** summarizes level/proficiency changes, spell-limit/slot changes, deterministic traits applied and any remaining spell or ASI/Feat choices without retaining progression metadata or choosing arbitrary player options.
...
- **Weapon combat:** STR/DEX/finesse selection, proficiency, magic bonuses, advantage/disadvantage, versatile damage, extra damage dice, riders and critical dice doubling. Ammunition weapons use their explicit per-weapon counter when configured; bows/crossbows with a zero counter consume matching loose Inventory ammunition stacks by ammunition group.
...
- Campaign Diagnostics opens first and runs only on explicit OK; its scene/link scan uses bounded temporary heap lines and one-row rendering to keep the 4 KB Adventure stack conservative.
