...
- **Level progression:** standard-array defaults, explicit ASI/Feat level choices, fixed-average Hit Die HP advancement, Hit Dice refresh and explicit deterministic grants. Raising a class level updates HP/Hit Dice and other level-derived rules but never writes progression traits. **Grant Initial Traits** applies only starting/level-1 species/background/class/subclass grants; **Apply Level Grants** explicitly catches up all currently eligible deterministic species/class/subclass grants. Both repair missing deterministic Feature/spell payloads when stale applied markers exist without duplicating records that are actually present, treat marker files as best-effort bookkeeping, and report `Updated` or `No changes` after an explicit grant action. Level Choices always opens and reports when no ASI/Feat choice is pending. A bounded post-level **Level-Up Review** summarizes level/proficiency, spell-limit/slot and pending-choice changes without retaining progression metadata or choosing arbitrary player options.
...
- **Features & Perks:** add/edit persistent paged features with uses, recharge cadence (Manual/Turn/Encounter/Dawn/Short-Long/Long), class metadata and resource formulas. Progression/grant feat selection opens in **Allowed** mode and Hold OK toggles Allowed/All; Allowed is conservative and contains only bundled feats whose represented prerequisites can be positively validated (level, Grappler ability scores, Fighting Style/Spellcasting Feature gates, Epic Boons and already-owned non-repeatable feats). Custom/unrecognized perks remain available through All, and Allowed fails closed if duplicate eligibility cannot be verified from the Feature sidecar. Manual Feature/Perk editing remains unrestricted. During a pending level choice, `Ability Score Improvement` is not offered inside the nested feat picker because the dedicated ASI +2 and +1/+1 choices own that rule effect.
...
- **Weapon combat:** STR/DEX/finesse selection, proficiency, magic bonuses, advantage/disadvantage, versatile damage, extra damage dice, riders and critical dice doubling. Ammunition weapons use their explicit per-weapon counter when configured; otherwise loose Inventory ammunition is matched case-insensitively by ammo token anywhere in the Item name, so custom stacks such as `Fire Arrow` can feed a bow.
...
- **Add/Edit/Delete Item:** blank record opens the full **36-field** editor immediately; Item Name can use the streamed catalog or a custom name. Item data covers quantity, weight, detail, equipped/attuned state, weapon/armor fields, damage, properties, ammunition, charges, containers and source metadata. A truly empty, never-granted Inventory automatically receives the normal initial equipment package on entry; an `InitialInventory` marker prevents silent duplication if the player later empties it.
...
- **Starting inventory:** one-time class/species/background grant; a truly empty, never-granted Inventory applies the initial package on entry. Items, prior balance + granted currency and grant marker publish in one synced sidecar transaction. A previously granted Inventory that was later emptied is not silently duplicated; the deliberate one-time regrant remains explicit. A d100 trinket is fallback only when normal composition yields neither items nor currency.
...
- Bundled 448-spell catalog ordered by spell level and then name, with class, level, school, ritual, source, status and eligibility filtering.
- **Class filtering:** **Character Classes** is the default and represents the union of the active character's actual spell lists. The selector also provides **Any Class** and every supported class name independent of the character build. `Allowed` still enforces actual character list/level access; `All Spells` turns the selected class into a pure catalog filter, and `Any Class + All Spells` exposes the complete catalog. Eldritch Knight and Arcane Trickster use the Wizard spell list.
...
- Adventure scene preview uses a 22-character text width. Hold OK opens a five-line, scrollable full-text scene viewer; Hold Left loads the saved checkpoint and Hold Right saves the current scene as the checkpoint. Restart is an explicit Campaign-menu action with confirmation and resets campaign scene/checkpoint/flags/achievements.
- Adventure campaign selection contains bundled/enabled campaigns plus the confirmed **Restart Current Adventure** action. The broken Campaign Diagnostics and Installed Pack Controls screens are intentionally absent; enabled pack content is still discovered through the normal bounded campaign loader.
...
Generate Encounter is the second Bestiary home row, followed immediately by Party Level and Party Size; Pack Diagnostics is last. Monster Pack Controls is intentionally absent while normal bundled/custom/enabled pack loading remains available.
...
- **Monster packs:** bundled/custom/enabled pack content remains storage-backed and available to browsing/generation. **Pack Diagnostics** retains validation/recovery at the end of the home menu; the broken Monster Pack Controls UI is intentionally removed.
