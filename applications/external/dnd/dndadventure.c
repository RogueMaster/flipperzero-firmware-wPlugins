...
static void adventure_reward_item(DndAdventureApp* app, const char* name) {
    ...
    PocketCharacter* character = &app->character_data.character;
    uint8_t total = 0U;
    ...
    for(uint8_t start = 0U; first_page || start < total; start += POCKET_D20_COLLECTION_CACHE_SIZE) {
        ...
        if(!pocket_d20_storage_load_items_window(
               app->storage, app->profile, start, character, &total)) {
            adventure_set_status(app, "Item storage read failed");
        ...
        for(uint8_t local = 0U; local < character->item_count; ++local) {
            ...
            if(character->items[local].quantity < 999) {
                ...
                if(!pocket_d20_storage_save_items_window(
                       app->storage, app->profile, start, character))
    ...
    if(total < POCKET_D20_MAX_ITEMS) {
        ...
        adventure_copy(item.name, sizeof(item.name), name);
        adventure_copy(item.detail, sizeof(item.detail), "Adventure reward");
        item.quantity = 1;
        item.container_index = -1;
        item.armor_dex_cap = -1;
        if(!pocket_d20_storage_append_item(app->storage, app->profile, &item))
            adventure_set_status(app, "Item reward save failed");
    }
}
