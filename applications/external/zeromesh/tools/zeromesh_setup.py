...
def run(args):
    print(
        "  $ "
        + " ".join(os.path.basename(a) if a.endswith(".py") else a for a in args[1:])
    )
...
def upload(port, local, remote, mkdir="/ext/apps_data/zeromesh"):
    ...
    if size > SERIAL_WARN_BYTES:
        print(
            "  note: %.1f MB over USB takes a while. A card reader is much faster."
            % (size / 1048576.0)
        )
        ...
        with open(local, "rb") as fh:
            while True:
                ...
                if not block:
                    break
                s.write(
                    ('storage write_chunk "%s" %d\r' % (remote, len(block))).encode()
                )
                s.read_until(b"\n")
                s.write(block)
                s.read_until(b">: ")
...
def send_ringtone(path, port):
    ...
    upload(
        dev,
        path,
        "/ext/apps_data/zeromesh/ringtones/" + name,
...
def main():
    ...
    run(
        [
            sys.executable,
            os.path.join(HERE, "fetch_tiles.py"),
            args.tiles,
            "--bbox",
            "%f,%f,%f,%f" % (w, s, e, n),
            "--min-zoom",
    ...
    run(
        [
            sys.executable,
            os.path.join(HERE, "build_pmtiles.py"),
            args.tiles,
            args.out,
            "--min-zoom",
            str(args.min_zoom),
    ...
    if not args.install:
        print()
        print(
            "Copy %s to /ext/apps_data/zeromesh/map.pmtiles on the SD card," % args.out
        )
