...
static void transform(CkSha256* context, const uint8_t block[64]) {
    ...
    for(size_t i = 0; i < 16U; ++i) words[i] = get_u32(block + i * 4U);
    for(size_t i = 16U; i < 64U; ++i) {
        uint32_t s0 = rotr(words[i - 15U], 7) ^ rotr(words[i - 15U], 18) ^
                      (words[i - 15U] >> 3);
        uint32_t s1 = rotr(words[i - 2U], 17) ^ rotr(words[i - 2U], 19) ^
                      (words[i - 2U] >> 10);
...
void ck_sha256_final(CkSha256* context, uint8_t digest[32]) {
    ...
    transform(context, context->block);
    for(size_t i = 0; i < 8U; ++i) put_u32(digest + i * 4U, context->state[i]);
