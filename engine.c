...
// 绘制 XBM 位图(预生成的中文字符)
void fb_blit_xbm(int x0, int y0, int w, int h, int bpr, const uint8_t *bits,
                 uint8_t on) {
  for (int y = 0; y < h; y++) {
    if (y0 + y < 0 || y0 + y >= SCREEN_H)
      continue;
    for (int x = 0; x < w; x++) {
...
static inline uint8_t map_at(int x, int y) {
  if ((unsigned)x >= (unsigned)g.map_w || (unsigned)y >= (unsigned)g.map_h)
    return WALL_BRICK;
...
void engine_render(void) {
  ...
  for (int ci = 0; ci < RENDER_COLS_DYN; ci++) {
    ...
    if (!hit) {
      ...
      for (int y = 0; y < SCREEN_H; y++) {
        uint8_t on =
            (y < SCREEN_H / 2) ? ceil_px(px_ctr, y) : floor_px(px_ctr, y);
  ...
  //    做法: 遍历玩家附近 5x5 格,非空道具格做简单投影 (z-buffer 简单近似)
  {
    ...
    Player *pp = &g.player;
    const float invDet =
        1.0f / (pp->plane_x * pp->dir_y - pp->dir_x * pp->plane_y);
    for (int y = -3; y <= 3; y++) {
      for (int x = -3; x <= 3; x++) {
