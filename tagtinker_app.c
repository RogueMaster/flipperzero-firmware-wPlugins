...
void tagtinker_prepare_text_tx(TagTinkerApp* app, const uint8_t plid[4]) {
    ...
    /* HD tags map UI page 1-8 to protocol 0-7. Color 2.6 uses the UI
     * page as the protocol page (store image slot is page 2). */
    if(app->selected_target >= 0 &&
       ...
       app->targets[app->selected_target].profile.type_code == 1626) {
        ...
        if(app->image_tx_job.page < 1U) app->image_tx_job.page = 1U;
    } else {
...
void tagtinker_prepare_bmp_tx(
    ...
    if(app->selected_target >= 0 &&
       app->selected_target < app->target_count &&
       app->targets[app->selected_target].profile.type_code == 1626 &&
       app->image_tx_job.page == 0U) {
        app->image_tx_job.page = 2U;
    }
