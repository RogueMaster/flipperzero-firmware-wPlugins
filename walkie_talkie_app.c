...
static const uint32_t pmr446_frequencies[PMR446_NUM_CHANNELS] = {
    446006250, 446018750, 446031250, 446043750, 446056250,
    446068750, 446081250, 446093750, 446106250, 446118750,
    446131250, 446143750, 446156250, 446168750, 446181250,
    446193750};
...
static void walkie_talkie_process_scanning(WalkieTalkieApp* app) {
    ...
    if(app->scan_direction == ScanDirectionUp) {
        ...
        app->current_channel =
            (app->current_channel > 0) ? app->current_channel - 1 : channel_count - 1;
    }
    walkie_talkie_tune_current_channel(app);
}
            ...
            if(event.type == InputTypeShort) {
                if(event.key == InputKeyOk) {
                    ...
                    if(app->page == WalkieTalkiePageMenu) {
                        ...
                        uint32_t last_item =
                            app->auto_squelch ? SettingsItemSensitivity : SettingsItemAutoSquelch;
