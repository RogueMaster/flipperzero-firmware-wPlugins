...
int test_scan_ctl_run(void) {
     ...
     */
    {
        ...
        CHECK(act_start == 2);
        CHECK(act_stop == 4);
        CHECK(sr_scan_ctl_retry_unconfirmed(
                  SrScanActNone, SrScanUiBusy, true, false) == SrScanActSendStop);
        CHECK(sr_scan_ctl_retry_unconfirmed(
                  SrScanActNone, SrScanUiBusy, true, true) == SrScanActNone);
        CHECK(sr_scan_ctl_retry_unconfirmed(
                  SrScanActNone, SrScanUiBusy, false, false) == SrScanActNone);
        CHECK(on_ok_total == 7);
    }
    ...
    CHECK(sr_scan_ctl_on_ok_upload_gate(
              SrScanUiIdle, false, false, false, false) == SrScanActSendStart);
    CHECK(sr_scan_ctl_on_ok_upload_gate(
              SrScanUiIdle, false, false, false, true) == SrScanActNone);
    CHECK(sr_scan_ctl_on_ok_upload_gate(
              SrScanUiStartFailed, false, false, false, true) == SrScanActNone);
    /* Must not send stopscan while the board is UPLOADING. */
    CHECK(sr_scan_ctl_on_ok_upload_gate(
              SrScanUiRunning, false, false, false, true) == SrScanActNone);
    CHECK(sr_scan_ctl_on_ok_ex(SrScanUiRunning, false, true, false) == SrScanActSendStop);
