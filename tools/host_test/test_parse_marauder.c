...
static bool oracle_sess_parse(const char* line, size_t len, OracleSess* o) {
    ...
    if(ap > 0xFFFFFFFFul || ble > 0xFFFFFFFFul || ms > 0xFFFFFFFFul ||
       ih > 0xFFFFFFFFul || ihmin > 0xFFFFFFFFul || ps > 0xFFFFFFFFul ||
       psmin > 0xFFFFFFFFul) {
        return false;
    }
    o->ap = (uint32_t)ap;
...
static void test_sess(void) {
    ...
    memset(&p, 0, sizeof(p));
    CHECK(feed_lit(&p, "Sess: ap=37 ble=5 ms=0 ih=1 ihmin=1 ps=1 psmin=1", &ev) ==
          SrParseOk);
    CHECK(ev.kind == SrEventSess);
