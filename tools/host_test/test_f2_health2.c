...
static void t_periodic_refresh_no_readopt_reseed(void) {
    ...
    for(i = 0; i < 3; i++) {
        ...
        srev++;
        sa = sr_sess_seed_eval(
            srev, srev - 1u, 60000u, SrSessionRunning, false /* seed_pending */);
...
static void t_stale_qual_degrades(void) {
    ...
    fresh_at_bound =
        sr_fmt_qual_fresh(1u /* qual_rev */, 1000u /* qual_tick_ms */, 5000u /* sess_ms */,
...
static void t_no_board_session_suppresses_verdict(void) {
    ...
    fresh = sr_fmt_qual_fresh(1u /* qual_rev */, 9000u /* qual_tick_ms */, 0u /* sess_ms */, 9000u);
    CHECK(fresh == false);
