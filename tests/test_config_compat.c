...
int main(void) {
    MorseFlipperListeningSettings source = {
        ...
        .custom_set_idx = 7U, .farnsworth_wpm = 19U, .answer_timeout_s = 9U,
        .group_pause_s = 14U};
    ...
    assert(bytes[8] == 57U && bytes[9] == 0U && bytes[15] == 7U);
    ...
    assert(!mf_config_test_load(bytes, &loaded));
    bytes[0] = 1U;
    bytes[4] = 0U; bytes[5] = 99U; bytes[6] = 1U; bytes[8] = 0U; bytes[15] = 9U;
    bytes[20] = 31U; bytes[21] = 2U; bytes[22] = 16U;
    assert(mf_config_test_load(bytes, &loaded));
    assert(loaded.local_dit_ms == 100U && loaded.lesson == 1U && loaded.group_size == 1U);
    assert(loaded.session_groups == 3U && loaded.custom_set_idx == 0U);
