...
INK_ASCENT = {"FontSecondary": 7, "FontPrimary": 8, "FontKeyboard": 7, "FontBigNumbers": 18}
...
for fn in sorted(os.listdir(VIEWS)):
    ...
    if hits:
        ...
        for a, b, gap, want in hits:
            at, ab = ink(a)
            bt, bb = ink(b)
            print(f"  {'TOUCHING' if gap == 0 else 'tight'}: {gap} blank row(s), wanted {want}")
...
if "--max" in sys.argv:
    ...
    if problems > limit:
        print(f"FAIL: {problems} overlap candidates, baseline is {limit}. Adjudicate the new one(s).")
        failed = True
    else:
...
if "--max-tight" in sys.argv:
    ...
    if tight > limit:
        print(f"FAIL: {tight} tight-clearance candidates, baseline is {limit}. Adjudicate the new one(s).")
        failed = True
    else:
