...
static bool rad_sens_read_register(uint8_t reg, uint8_t* data, size_t data_size, uint32_t timeout) {
...
bool rad_sens_set_sensitivity(uint16_t sensitivity) {
    ...
    if(furi_hal_i2c_is_device_ready(I2C_BUS, RAD_SENS_ADDRESS, timeout) > 0) {
        success =
            rad_sens_write_register(RAD_SENS_SENSITIVITY_RG, sensitivity & 0xFF, timeout);
        if(success) {
            furi_delay_ms(20);
