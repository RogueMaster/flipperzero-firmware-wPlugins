...
static inline uint32_t _nsh_div8(uint32_t x) { return x >> 3; }
static inline uint32_t _nsh_mod8(uint32_t x) { return x & 7u; }
...
bool nsh_map_set(uint32_t block_number, uint8_t value) {     // Set numbered block to 0/1, return false if out of range
...
int nsh_parts_get(uint32_t part_index) {
    ...
    return (nsh_parts.total[part_index] > 0 &&
            nsh_parts.received[part_index] >= nsh_parts.total[part_index]) ? 1 : 0;
}
...
bool nsh_parts_is_ready(void) {
    ...
    return nsh_parts.mode == NSH_PARTS_MODE_BLOCKS_PER_PART || nsh_parts.mode == NSH_PARTS_MODE_PARTS_PER_BLOCK;
}
...
void nsh_ensure_inbox_dir(void) {
    ...
    if(!ok) {
        FURI_LOG_E(TAG, "nsh_ensure_inbox_dir: Failed to create directory '%s'", EXT_PATH(NSH_RECEIVER_DIRECTORY));
    }
...
bool nsh_init(const nsh_init_params_t* p) {
    ...
    if (p->mode == NSH_MODE_SENDER) {
        ...
        if (!pfile || !storage_file_open(pfile, p->s_file_path, FSAM_READ, FSOM_OPEN_EXISTING)) {
            FURI_LOG_E(TAG, "nsh_init: Failed to open persistent read handle '%s'", p->s_file_path);
        ...
        FURI_LOG_I(TAG, "nsh_init: SENDER, file_path='%s', file_name='%s', file_size=%lu, tx_id=%d",
            g.s_file_path, g.s_file_name, g.s_file_size, g.tx_id);
        FURI_LOG_I(TAG, "nsh_init: md5=%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X",
...
void nsh_send_announce(void) {
    ...
    FURI_LOG_I(TAG, "nsh_send_announce: file_name='%s', file_size=%lu", g.s_file_name, g.s_file_size);
    ...
    if (g.mode == NSH_MODE_RECEIVER) {
        ...
        if (now - g.last_rx_ms > NSH_RX_TIMEOUT_MS) {
            if (g.r_locked && !g.r_finalizing && g.r_blocks_received < g.r_blocks_needed) {
                ...
                FURI_LOG_I(TAG, "nsh_idle: REQUEST blocks (%lu, %lu), bytes (%lu, %lu)",
        ...
        if (do_finalize) {
            ...
            if(!(furi_thread_flags_get() & NSH_WORKER_STOP_FLAG)) {
                if (success) nsh_notify_success();
                else nsh_notify_error();
            }
        }
        return;
    }
...
static void nsh_handle_announce(uint8_t tx_id, const NSH_pl_announce_t* ann) {
    ...
    if (g.r_locked == false) {
        ...
        FURI_LOG_I(TAG, "nsh_handle_announce: LOCK to tx_id=%d, file_name='%s', file_size=%lu",
        ...
        snprintf((char*)g.r_file_path, sizeof(g.r_file_path), "/ext/%s/%s", NSH_RECEIVER_DIRECTORY, g.r_file_name);
        ...
        FURI_LOG_I(TAG, "nsh_handle_announce: Init nsh_parts for %lu blocks...", g.r_blocks_needed);
    ...
    if (notify_red)   nsh_notify_led_red();
    else if (notify_blue) nsh_notify_led_blue();
}
...
static void nsh_handle_data(uint8_t tx_id, const NSH_pl_data_t* d) {
    ...
    if (g.r_finalizing || g.r_is_finished) { nsh_unlock(); return; }

    g.last_rx_ms = g.cb_now_ms ? g.cb_now_ms() : 0;

    if (!g.r_locked || tx_id != g.r_locked_tx_id) { nsh_unlock(); return; }
