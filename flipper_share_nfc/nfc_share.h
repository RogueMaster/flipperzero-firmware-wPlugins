...
// ===== Behaviour tunables ====================================================
#define NSH_PAYLOAD_THROUGHPUT_BPS 4000                      // nominal NFC payload throughput for ETA, bytes/sec (estimate; update after hardware measurement)
...
typedef struct {
    ...
    uint32_t (*cb_read_block)(uint32_t block_number, uint8_t out[NSH_DATA_LENGTH]);        // Reader callback from real storage
    ...
    void (*cb_write_block)(uint32_t block_number, const uint8_t in[NSH_DATA_LENGTH], uint32_t valid_len);
} nsh_ctx_t;
...
typedef struct {
    ...
    void (*r_write_block)(uint32_t block_number, const uint8_t in[NSH_DATA_LENGTH], uint32_t valid_len);
} nsh_init_params_t;
